...
- 新增 Stop transaction remediation 测试覆盖（+870 行）：watchdog 触发后连接毒化与重建、soft timeout / ReadFrame 错误的 conn 失效、expectedConn 比较避免误杀新连接（详见 `docs/plans/2026-07-31-wispa-stop-transaction-remediation.zh-CN.md`）。
...
### Added
- 设备配置新增可选本地 IP 绑定，多网卡 Windows 工位可指定连接 WISPA 使用的网卡。
...
### Added
- 新增 WISPA 全压力通道校零功能：顶栏右侧以校零按钮替换配置按钮，当前选中设备处于已连接或采集中时均可发送设备原生 `h` 校零命令。
...
- WISPA 设备固件时间戳 bug 仍存在，CSV 时间戳已统一截断到秒级规避。

## [0.5.0] - 2026-07-25

### Internal
- 与 daq-t1603 版本号同步发布。本版本 wispa 无独立功能改动：最近共享 SDK 改动（`shared/device-sdk/go/daq/hardware/daq_t1603.go` 的 drainConnection 连续静默窗口语义）仅影响 T1603 设备，P1604 使用 w1601 长度前缀协议且 ReadLoop 路径不同，不受影响。
...
### Verification
- `task release`（clean + 前端 build + 生产 Go 构建）：通过。前端 `npm run build`（vue-tsc + vite）成功，`go build -tags production -trimpath -buildvcs=false -ldflags="-w -s -H windowsgui"` 产出 `build/bin/wispa.exe`。
- `go vet ./...`（GOWORK=off）：passed。
- `go test ./...`（GOWORK=off）：passed。
- `makensis /DARG_WAILS_AMD64_BINARY=..\..\bin\wispa.exe project.nsi`：产出 `wispa-0.5.0-amd64-installer.exe`，归档至 `releases/bin/`。
...
- `makensis /DARG_WAILS_AMD64_BINARY=..\..\bin\wispa.exe project.nsi`：产出 `wispa-0.4.0-amd64-installer.exe`，归档至 `releases/bin/`。
...
- `npm run build`
- `task release`
- `makensis /DARG_WAILS_AMD64_BINARY=build/bin/wispa.exe project.nsi`
...
### Fixed
- 修复 CSV Timestamp 列时间戳错误：WISPA 设备硬件时间戳存在固件 bug（fractional 字段以 ~4348Hz 速率递增，每累积约 232ms 跳跃校正），导致 1000Hz 采集下 1 毫秒内出现多帧时间戳；系统毫秒时间戳在 1000Hz 下精度也不足。统一截断到秒级，避免展示错误的时间细分。

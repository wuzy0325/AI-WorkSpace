...
> **适用范围**：所有使用 Wails v3 + WebView2 渲染的桌面应用（wispa / wista / windlabx4 / motion-controller 等）。本文以 `wispa` 为具体示例，方法论与代码模板可直接复用。
>
...
| 项目 | 环境变量 | 推荐端口 |
|---|---|---|
| wispa | `wispa_CDP_PORT` | 9222 |
| wista | `DAQ_T1603_CDP_PORT` | 9223 |
...
- 真实设备已上电、IP 可达（如 wispa 设备 `192.168.1.7:9000`）。
...
$env:GOWORK="off"   # wispa 在 go.work 中，按需设置
go build -o wispa.exe .
```
...
```powershell
# 先杀掉可能残留的旧进程
Get-Process wispa -ErrorAction SilentlyContinue | Stop-Process -Force

# 设置 CDP 端口并启动
$env:wispa_CDP_PORT="9222"
.\wispa.exe
```

> **注意**：`wispa.exe` 是阻塞进程，需在独立终端启动，或用 `Start-Process` 后台启动。AI Agent 建议用 `RunCommand` 的 `blocking: false` + `command_type: "long_running_process"`。
...
预期返回 1 个 page，title 为应用窗口标题（如 "WISPA 压力采集"）。
...
> **路径陷阱**：`Path(__file__).parent.parent` 从 `apps/desktop-wails/` 上溯两级是 `apps/`，所以截图落在 `apps/docs/`。要落到 `projects/wispa/docs/`，需 `parent.parent.parent` 或显式指定绝对路径。**建议用绝对路径或相对仓库根**。
...
完整脚本参见 [e2e_acquisition.py](file:///c:/Users/wuzhy/Documents/D/SVN/SoftWare/trunk/AI-Workspace/projects/wispa/apps/desktop-wails/e2e_acquisition.py)。
...
[E2E] 15:38:14 已选中设备: WISPA-374D87
...
如需把截图嵌入报告或聊天展示（避免 PNG 过大），可缩成 JPEG + base64。脚本见 [resize_shots.ps1](file:///c:/Users/wuzhy/Documents/D/SVN/SoftWare/trunk/AI-Workspace/projects/wispa/apps/desktop-wails/resize_shots.ps1)。
...
- [ ] 脚本退出码为 0
- [ ] 日志出现 "E2E 测试全部通过 ✓"
- [ ] 所有步骤截图已生成（按 [第 5.5 节](#55-完整测试链路wispa-实例)表格）
...
2. **改环境变量名**：`wispa_CDP_PORT` → `XXX_CDP_PORT`（见 [第 2.4 节](#24-环境变量命名约定)）。
...
**维护者**：本文档基于 2026-07-23 wispa 真实设备 E2E 测试经验整理。Wails v3 版本以 `v3.0.0-alpha2.106` 为准；后续 Wails 升级若改变 `AdditionalBrowserArgs` 字段位置，需同步更新 [第 2.2 节](#22-正确做法wails-v3-的-windowsoptionsadditionalbrowserargs)。

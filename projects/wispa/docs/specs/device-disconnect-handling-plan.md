# Implementation Plan: daq-p1604 设备断连处理改进

> **关联 spec**：[device-disconnect-handling.md](file:///c:/Users/wuzhy/Documents/D/SVN/SoftWare/trunk/AI-Workspace/projects/daq-p1604/docs/specs/device-disconnect-handling.md)
...
- [ ] **Task 1: 启用 TCP keepalive 加速断连检测**
  ...
  - **Verification**:
    - [ ] `cd projects\daq-p1604\apps\desktop-wails; $env:GOWORK="off"; go test ./adapters/hardware/ -run TestEnableTCPKeepalive -v`
    - [ ] `go vet ./...`
    - [ ] `go build ./...`
  - **Dependencies**: None
...
- [ ] **Task 5: 端到端集成验证 + 回归测试**
  ...
  - **Verification**:
    ...
    - [ ] `npm run typecheck` + `npm run build` 全绿
    - [ ] `powershell -File .\scripts\check-wails-bindings.ps1 -Projects daq-p1604`

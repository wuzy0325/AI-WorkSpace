...
// idRequest 是只含 id 字段的请求体，用于 connect/disconnect/start/stop 等。
type idRequest struct {
	ID string `json:"id"`
}

// decodeIDRequest 解析 {"id":"..."} 请求体并校验非空。
// 返回 (id, true) 表示成功；返回 ("", false) 表示已写错误响应，调用方应 return。
func decodeIDRequest(w http.ResponseWriter, r *http.Request) (string, bool) {
	var body idRequest

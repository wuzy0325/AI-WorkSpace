...
type P1604Config struct {
	...
	UseDeviceTimestamp *bool  `json:"useDeviceTimestamp,omitempty"` // 是否使用设备硬件时间戳，nil=默认开启（兼容老 profile），显式 false=用系统时间
}

// UseDeviceTimestampEnabled 返回是否启用设备硬件时间戳
// nil 视为 true（默认开启），与历史行为一致；显式设置时返回指针值
func (c P1604Config) UseDeviceTimestampEnabled() bool {
	if c.UseDeviceTimestamp == nil {
		return true
	}
	return *c.UseDeviceTimestamp
}

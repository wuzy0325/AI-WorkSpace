...
//   - 保留单体 App（wista 拆为 3 个 Service），main.go 只调用一次 ServiceStartup/Shutdown
...
// 端口 18182：与 wista（18181）区分，避免同机双开时端口冲突。
...
func main() {
	...
	if configDir != "" {
		configDir = filepath.Join(configDir, "wispa")
	} else {
		configDir = "config"
	}
	...
	//   - WISPA_MODE=simulated → 用 SimulatedAdapter + SimulatedScanner

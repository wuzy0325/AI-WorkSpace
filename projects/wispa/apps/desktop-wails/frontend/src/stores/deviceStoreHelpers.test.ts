...
describe('dedupeName', () => {
  it('目标名字不冲突时原样返回', () => {
    expect(dedupeName('WISPA-3A5F1B', new Set())).toBe('WISPA-3A5F1B')
  })

  it('冲突一次追加 (2)', () => {
    expect(dedupeName('WISPA-3A5F1B', new Set(['WISPA-3A5F1B'])))
      .toBe('WISPA-3A5F1B (2)')
  })

  it('冲突到 (2) 时继续升到 (3)', () => {
    const taken = new Set(['WISPA-3A5F1B', 'WISPA-3A5F1B (2)'])
    expect(dedupeName('WISPA-3A5F1B', taken)).toBe('WISPA-3A5F1B (3)')
  })
})
...
describe('planScannedAdditions', () => {
  it('全新设备全部通过，名字互不相同', () => {
    ...
    expect(plan.toAdd[0]!.name).toBe('WISPA-3A5F1B')
    expect(plan.toAdd[1]!.name).toBe('WISPA-9C82AA')
  ...
  it('两台 MAC 末6 相同时第二台名字自动追加 (2)', () => {
    ...
    expect(plan.toAdd[0]!.name).toBe('WISPA-3A5F1B')
    expect(plan.toAdd[1]!.name).toBe('WISPA-3A5F1B (2)')
  ...
  it('新名字与已有 profile 名字冲突时也会追加 (2)', () => {
    const plan = planScannedAdditions({
      ...
      existingProfiles: [
        { id: 'other', name: 'WISPA-3A5F1B', address: '192.168.3.99', port: 9000 } as any,
      ],
      defaultAutoConnect: true,
    })
    expect(plan.toAdd[0]!.name).toBe('WISPA-3A5F1B (2)')
  })

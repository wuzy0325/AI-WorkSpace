// Package backend 提供 WISPA 后端应用入口。
//
...
//
// 与 wista 的差异：
//   - 保留单体 App 设计（wista 拆为 Device/Recording/Log 3 个 Service），
//     因为 wispa 业务方法间紧耦合（handleRelayExit 同时访问 deviceUC + recordUC）
//   - latestSnapshots 字段保留在 App 内（前端 500ms 轮询 GetLatestSnapshot 用），
//     wista 改为通过 daq:payload 事件推送不需要此字段
package backend
...
func (a *App) ServiceStartup(ctx context.Context) error {
	...
	slog.Info("WISPA application started")
...
func (a *App) ServiceShutdown() error {
	...
	a.EmitLog(core.LogEvent{
		...
		Category: "system",
		Source:   "app",
		Message:  "WISPA application shut down",
	})
	slog.Info("WISPA application shut down")
	return nil
}
...
//
// 与 wista 的差异：
//   - wista 的 relay 还负责 100ms 节流推送 daq:payload 事件
//   - wispa 不推 payload 事件，仅更新 latestSnapshots 供前端轮询
//
// defer 顺序说明（LIFO）：

...
func (s *winsockDiscoverySocket) Send(data []byte, target string, port int) error {
	...
	sendMs := int(discoverySendTimeout.Milliseconds())
	if sendMs < 1 {
		sendMs = 1
	}
	if err := windows.SetsockoptInt(handle, windows.SOL_SOCKET, soSNDTIMEO, sendMs); err != nil {
		return err
	}
...
func (s *winsockDiscoverySocket) Receive(buf []byte, timeout time.Duration) (int, string, error) {
	...
	recvMs := int(timeout.Milliseconds())
	if recvMs < 1 {
		recvMs = 1
	}
	if err := windows.SetsockoptInt(handle, windows.SOL_SOCKET, windows.SO_RCVTIMEO, recvMs); err != nil {
		return 0, "", err
	}

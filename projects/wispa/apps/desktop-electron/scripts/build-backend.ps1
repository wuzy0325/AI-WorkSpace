...
#   3. exe 输出到 desktop-electron/backend/wispa-backend.exe
#
# 注意事项：
...
try {
    & (Join-Path $goRoot 'bin\go.exe') build -buildvcs=false -trimpath -ldflags '-s -w -H=windowsgui' -o (Join-Path $backendOutput 'wispa-backend.exe') .

...
// 端口 18182：与 wista（18181）区分，避免同机双开冲突。
...
function backendPath() {
  if (app.isPackaged) {
    return path.join(process.resourcesPath, 'backend', 'wispa-backend.exe')
  }
  return path.join(__dirname, 'backend', 'wispa-backend.exe')
}
...
function startBackend() {
  ...
  backendProcess.once('error', (error) => {
    dialog.showErrorBox('WISPA 后端启动失败', error.message)
    app.quit()
  })
  backendProcess.once('exit', (code) => {
    ...
    if (!app.isQuitting && code !== 0) {
      dialog.showErrorBox('WISPA 后端异常退出', `退出代码: ${code}`)
      app.quit()
    }
  })
}
...
app.whenReady().then(async () => {
  ...
  try {
    ...
    dialog.showErrorBox('WISPA 启动失败', error.message)
    app.quit()
  }
})

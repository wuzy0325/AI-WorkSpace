...
- `task release`: passed.
- `makensis '-DARG_WAILS_AMD64_BINARY=..\..\bin\wista.exe' project.nsi`（在 `build/windows/installer/` 目录下执行）: passed.
...
- `$env:GOWORK="off"; go build -tags production -trimpath -buildvcs=false -ldflags="-w -s -H windowsgui" -o build/bin/wista.exe .`
- `$env:GOWORK="off"; go vet ./...`
- `npm run typecheck`
- `npm run build`
- `task release`
...
### Verification
- 生产 Go 构建 `go build -tags production -trimpath -buildvcs=false -ldflags="-w -s -H windowsgui"`：通过，产出 `build/bin/wista.exe`。
- `go vet ./...`（GOWORK=off）：passed。
- `go test ./...`（GOWORK=off）：passed（adapters/config、adapters/hardware、adapters/recording、usecase 均 ok）。
- `makensis /DARG_WAILS_AMD64_BINARY=..\..\bin\wista.exe project.nsi`：产出 `wista-0.5.0-amd64-installer.exe`，归档至 `releases/bin/`。
...
- 同步 bindings（EmitDeviceState / SetDeviceProfile 导出）到 wista .ts bindings（wails3 运行时会重新生成为 .js 并丢弃提交的 .ts）。
...
- `go vet ./...`（GOWORK=off）：passed。
- `go test ./...`（GOWORK=off）：passed（adapters/config、adapters/hardware、adapters/recording、usecase 均 ok）。
- `makensis /DARG_WAILS_AMD64_BINARY=..\..\bin\wista.exe project.nsi`：产出 `wista-0.4.0-amd64-installer.exe`，归档至 `releases/bin/`。
...
- `$env:GOWORK="off"; go vet ./...`
- `task release`（手动 fallback：`go build -tags production` + `makensis`，wista 无 Taskfile）
...
### Verification
- `$env:GOWORK="off"; go build ./...`: passed（wista 工作空间隔离，见 ADR-006）

...
#
# 步骤：
#   1. cd 到 frontend 目录执行 npm run build（生成 frontend/dist，含 vue-tsc 类型检查）
#   2. cd 到 desktop-wails 目录执行 go build（embed frontend/dist 生成 exe）
#   3. exe 输出到 desktop-electron/backend/motion-controller-backend.exe
#
# 注意事项：
#   - 必须用 Go 1.20.14（最后支持 Win7 的 Go 版本），路径硬编码到 C:\go-versions\go1.20.14
#   - GOWORK=off：避免工作空间中其他 Go 1.21+ 模块（如 wind-daq services/api-go）的间接依赖冲突
#   - CGO_ENABLED=0：纯 Go 静态链接，避免依赖 mingw
#   - -ldflags '-s -w -H=windowsgui'：剥调试信息 + 标记为 GUI 程序（无控制台窗口）
#
# 与 probe-interpolator build-backend.ps1 的差异：
#   - 输出 exe 名为 motion-controller-backend.exe（与 main.cjs backendPath() 对应）
#   - 前端构建包含 tailwind/postcss，需确保 devDependencies 完整安装
...
try {
    & (Join-Path $goRoot 'bin\go.exe') build -buildvcs=false -trimpath -ldflags '-s -w -H=windowsgui' -o (Join-Path $backendOutput 'motion-controller-backend.exe') .
    if ($LASTEXITCODE -ne 0) {

...
!define INFO_PRODUCTVERSION "0.3.4"     # Default "{{.Info.ProductVersion}}"
...
# Chinese
LangString INST_WELCOME_TITLE  ${LANG_SIMPCHINESE} "欢迎使用运动控制器"
LangString INST_WELCOME_TEXT   ${LANG_SIMPCHINESE} "运动控制器 用于运动控制器的配置、测试与操作。$\r$\n$\r$\n主要功能：$\r$\n  * 运动控制器设备管理（B140 等）$\r$\n  * 轴运动控制与实时状态监控$\r$\n  * 运动参数配置与配置文件管理$\r$\n  * 独立于风洞主系统运行与调试$\r$\n$\r$\n安装程序将引导您完成安装。"
...
LangString INST_FINISH_TEXT    ${LANG_SIMPCHINESE} "运动控制器 已成功安装。$\r$\n$\r$\n点击完成退出安装程序。"
LangString INST_RUN_TEXT       ${LANG_SIMPCHINESE} "启动运动控制器"
# English
LangString INST_WELCOME_TITLE  ${LANG_ENGLISH} "Welcome to Motion Controller"
LangString INST_WELCOME_TEXT   ${LANG_ENGLISH} "Motion Controller is a standalone application for configuring, testing and operating motion controllers.$\r$\n$\r$\nKey features:$\r$\n  * Motion controller device management (B140, etc.)$\r$\n  * Axis motion control with real-time status monitoring$\r$\n  * Motion profile configuration and profile management$\r$\n  * Standalone operation independent of the wind tunnel main system$\r$\n$\r$\nThe wizard will guide you through the installation."
...
LangString INST_FINISH_TEXT    ${LANG_ENGLISH} "Motion Controller has been successfully installed.$\r$\n$\r$\nClick Finish to exit the setup wizard."
LangString INST_RUN_TEXT       ${LANG_ENGLISH} "Launch Motion Controller"
...
Section
    ...
    # Bundle motion controller hardware driver DLL
    !ifdef ARG_WAILS_AMD64_BINARY
        ${if} ${IsNativeAMD64}
            File "dlls\WTNMC4A_64.dll"
        ${EndIf}
    !endif

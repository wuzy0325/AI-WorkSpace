...
// ThreeHoleWorkspace.vue 是 3 孔探针工作区组件。
// 由 App.vue 在 activeProbe === 'three' 时动态加载。
// 内容从旧 three-hole-interpolator 项目的 App.vue 迁移而来：
//   - header / info-card / main / status-bar 完整保留
//   - adapter 引入路径从 ./wails-adapter 改为 ../adapters/three-hole
//   - 类型名 InterpolationInput → ThreeHoleInterpolationInput
//   - 类型名 InterpolationResult → ThreeHoleInterpolationResult
//   - 根类名 .app → .workspace-three（与 FiveHoleWorkspace 的 .workspace 平级，避免 CSS 互相覆盖）
//   - 样式沿用 FiveHoleWorkspace 的设计语言（SPEC 决策 UI(A)：共用 UI 框架）
//   - 状态栏文案 "三孔探针插值计算系统 v1.0" → "三孔探针工作区 v0.1.0"
import { ref, computed } from 'vue'
import {
  api,
  isWailsAvailable,
  type ThreeHolePrbFileInfo,
  type ThreeHoleInterpolationInput,
...
const prbFiles = ref<ThreeHolePrbFileInfo[]>([])
const machRange = ref<number[]>([])
const inputs = ref<ThreeHoleInterpolationInput[]>([])
const results = ref<(ThreeHoleInterpolationResult | null)[]>([])
...
// ==================== 数据操作 ====================
function buildCurrentInput(): ThreeHoleInterpolationInput {
  return {
    P1: newP1.value,
    P2: newP2.value,
    P3: newP3.value,
  ...
  } as ThreeHoleInterpolationInput
}

function isValidInput(input: ThreeHoleInterpolationInput): boolean {
...
async function importCsv() {
  ...
  try {
    ...
    // 3 孔 CSV 不带 pressureMode 列，导入后统一按当前模式填充。
    // 这样用户可在导入前通过模式开关决定这批数据按表压还是绝压处理。
    for (const d of data) {
...
function exportResults() {
  ...
  // 3 孔结果字段：α=迎角、Ma、Pt（总压）、Ps（静压）。无 β / V / Vx / Vy / Vz。
    ...
    <header class="app-header">
      <div class="header-brand">
        ...
        <div class="brand-text">
          <h1>三孔探针插值计算</h1>
          <span class="subtitle">Three-Hole Probe Interpolation</span>
        </div>
      </div>
      ...
      <Transition name="fade" mode="out-in">
        <div v-if="activeTab === 'input'" key="input" class="panel">
          ...
          <div class="input-section">
            ...
            <!-- 单行输入布局：3 孔只有 P1/P2/P3，加 Patm/Tatm/操作共 6 列 -->
            <div class="input-row">
              <div class="input-group">
                <label class="input-label">
                  <span class="label-text">P1</span>
                  <span class="label-hint">{{ pressureMode === 'gauge' ? '1号孔(表压)' : '1号孔(绝压)' }}</span>
                </label>
              ...
              <div class="input-group">
                <label class="input-label">
                  ...
                  <span class="label-hint">{{ pressureMode === 'gauge' ? '中心孔(表压)' : '中心孔(绝压)' }}</span>
                </label>
              ...
              <div class="input-group">
                <label class="input-label">
                  ...
                  <span class="label-hint">{{ pressureMode === 'gauge' ? '3号孔(表压)' : '3号孔(绝压)' }}</span>
                </label>
...
/* 工作区样式不带 scoped，与 App.vue 的全局基础样式配合。
   ...
   避免相互覆盖。当前样式与 FiveHoleWorkspace 一致（SPEC 决策 UI(A)：共用 UI 框架）。 */
.workspace-three { max-width: 1440px; margin: 0 auto; padding: 12px; min-height: 100vh; display: flex; flex-direction: column; gap: 10px; }
...
.col-mode { color: #4f46e5; font-weight: 600; font-size: 10px; }

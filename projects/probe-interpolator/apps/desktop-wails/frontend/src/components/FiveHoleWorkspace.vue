...
// FiveHoleWorkspace.vue 是 5 孔探针工作区组件。
// 由 App.vue 在 activeProbe === 'five' 时动态加载。
// 内容从旧 five-hole-interpolator 项目的 App.vue 迁移而来：
//   - header / info-card / main / status-bar 完整保留
//   - adapter 引入路径从 ./wails-adapter 改为 ../adapters/five-hole
...
import {
  ...
  isWailsAvailable,
  type PrbFileInfo,
  type FiveHoleInterpolationInput,
...
const inputs = ref<FiveHoleInterpolationInput[]>([])
const results = ref<(FiveHoleInterpolationResult | null)[]>([])
...
// resultVelocity 优先用 V 字段（探针坐标系合速度），回退到 velocity（标量）。
...
function buildCurrentInput(): FiveHoleInterpolationInput {
  ...
  } as FiveHoleInterpolationInput
}

function isValidInput(input: FiveHoleInterpolationInput): boolean {
...
function exportResults() {
  ...
  // 5 孔语义：α=迎角、β=侧滑角（与 7 孔反转，spec §2.2）。
  // 表头标注物理含义，避免与 7 孔 CSV 混用导致方向误判。
    ...
    <header class="app-header">
      <div class="header-brand">
        ...
        <div class="brand-text">
          <h1>五孔探针插值计算</h1>
          <span class="subtitle">Five-Hole Probe Interpolation</span>
        </div>
      </div>
...
/* 工作区样式不带 scoped，与 App.vue 的全局基础样式配合。
   ...
   避免相互覆盖。当前 .workspace 的内容样式与旧 5-hole App.vue 一致。 */
.workspace { max-width: 1440px; margin: 0 auto; padding: 12px; min-height: 100vh; display: flex; flex-direction: column; gap: 10px; }

...
//   - 后端地址固定为 http://127.0.0.1:18183（与 main.go listenAddr 对应）。
//
// 与原 Wails 绑定的差异：
//   - Wails 绑定是类型安全的（自动生成 .d.ts），HTTP 调用方需手动类型注解；
//   - Wails 绑定调用失败直接 reject 字符串，HTTP 客户端统一 reject Error 对象。
//
// 与 daq-p1604（18182）/ daq-t1603（18181）端口区分，避免同机多开冲突。

/** 后端 API 根地址（端口 18183，与 main.go listenAddr 对应） */
const API_BASE = 'http://127.0.0.1:18183'
 ...
 * @param path 路径，以 / 开头（如 /api/probe/available）

...
// LoadThreeHolePrbFiles 加载 3 孔 .prb 校准文件（通常单文件），
// 构建 ThreeHoleInterpolator 并缓存到 threeHoleState。
// 返回加载结果（文件列表 + 马赫数范围 + 警告）供前端展示。
//
// Win7 分支：文件路径由前端通过 Electron IPC 选择后传入，后端不再弹原生对话框。
func (a *App) LoadThreeHolePrbFiles(filePaths []string) ThreeHoleLoadPrbResponse {
	if len(filePaths) == 0 {
		return ThreeHoleLoadPrbResponse{Success: false, Error: "未选择文件"}
	}
...
// ImportThreeHoleCsvData 解析 3 孔数据 CSV/TXT/DAT，
// 自动检测分隔符（Tab 或逗号），解析 P1 + P2 + P3 + Patm + Tatm 列。
// 与 5 孔不同：3 孔的 Patm/Tatm 列为必填（与旧 3 孔程序一致，保证向后兼容）。
//
// Win7 分支：文件路径由前端通过 Electron IPC 选择后传入，后端不再弹原生对话框。
func (a *App) ImportThreeHoleCsvData(filePath string) ThreeHoleImportCsvDataResponse {
	if filePath == "" {
		return ThreeHoleImportCsvDataResponse{Success: false, Error: "未选择文件"}
	}

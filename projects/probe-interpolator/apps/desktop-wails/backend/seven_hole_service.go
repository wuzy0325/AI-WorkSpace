...
// LoadSevenHolePrbFiles 加载 7 个 .prb 校准文件（1.prb..7.prb），
// 构建 SevenHolePrbInterpolator 并缓存到 sevenHoleState。
//
...
func (a *App) LoadSevenHolePrbFiles(filePaths []string) SevenHoleLoadPrbResponse {
	if len(filePaths) == 0 {
		return SevenHoleLoadPrbResponse{Success: false, Error: "未选择文件"}
	}
...
// ImportSevenHoleCsvData 解析 7 孔数据 CSV，
...
//
// Win7 分支：文件路径由前端通过 Electron IPC 选择后传入，后端不再弹原生对话框。
func (a *App) ImportSevenHoleCsvData(filePath string) SevenHoleImportCsvDataResponse {
	if filePath == "" {
		return SevenHoleImportCsvDataResponse{Success: false, Error: "未选择文件"}
	}

...
#   3. exe 输出到 desktop-electron/backend/probe-interpolator-backend.exe
#
# 注意事项：
...
#
# 与 daq-p1604 build-backend.ps1 的差异：
#   - 输出 exe 名为 probe-interpolator-backend.exe（与 main.cjs backendPath() 对应）
#   - 前端无 vue-tsc 之外的额外测试步骤（探针插值器无单元测试）
...
try {
    & (Join-Path $goRoot 'bin\go.exe') build -buildvcs=false -trimpath -ldflags '-s -w -H=windowsgui' -o (Join-Path $backendOutput 'probe-interpolator-backend.exe') .

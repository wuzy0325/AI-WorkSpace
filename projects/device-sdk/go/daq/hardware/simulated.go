...
func NewSimulatedDevice(profile core.Profile) *SimulatedDevice {
...
func (d *SimulatedDevice) SetDataSink(sink core.DataSink) {
	d.mu.Lock()
	d.sink = sink
	d.mu.Unlock()
}
...
func (d *SimulatedDevice) Status() core.Status {
	d.mu.RLock()
	defer d.mu.RUnlock()
	return d.status
}

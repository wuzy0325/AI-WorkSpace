...
func NewDSA3217(profile core.Profile) *DSA3217 {
	return &DSA3217{
		profile: profile,
		status: core.Status{
			ID:         profile.ID,
...
func (d *DSA3217) SetDataSink(sink core.DataSink) {
	d.mu.Lock()
	d.sink = sink
	d.mu.Unlock()
}

func (d *DSA3217) Status() core.Status {
	d.mu.RLock()
	defer d.mu.RUnlock()
	return d.status
}
...
func (d *DSA3217) GetDsa3217ScanConfig() (core.DSA3217ScanConfig, error) {
	avg, period, unit, err := d.ReadScanConfig()
	if err != nil {
		return core.DSA3217ScanConfig{}, err
	}
	fps := "--"
	if avg >= 1 && period >= 73 {
		fps = fmt.Sprintf("%.3f", 1.0/(float64(period)*1e-6*16*float64(avg)))
	}
	return core.DSA3217ScanConfig{
		Avg:    avg,
		Period: period,
		Fps:    fps,
		Unit:   unit,
...
func (d *DSA3217) ApplyDsa3217ScanConfig(avg int, period int) (core.DSA3217ScanConfig, error) {

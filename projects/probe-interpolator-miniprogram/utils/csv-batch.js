...
//
// 输出表头约定（顺序固定）：
//   P1..Pn, Patm, TAtm, <resultColumns 各列>, isValid, warning
// 其中 isValid = 1/0，warning 为该行警告文本；解析/计算失败行 isValid=ERROR。
...
//   interp: 已加载的插值器实例
//   calculateRow(interp, input): 返回 result 对象（含 isValid/warning）；可抛出异常
...
function runBatch(text, opts) {
  ...
  const header = inCols.concat(['Patm', 'TAtm']).concat(resultColumns.map(resultColumnHeader)).concat(['isValid', 'warning']);
  ...
  for (const raw of parsed.rows) {
    ...
    if (bad) {
      errors++;
      const out = inCols.map(() => '').concat([fmt(input.PAtm), fmt(input.TAtm)])
        .concat(resultColumns.map(() => '')).concat(['ERROR', badMsg]);
      rows.push(out);
      continue;
    }
    ...
    if (errMsg) {
      errors++;
      const out = inCols.map((k) => fmt(input[k]))
        .concat([fmt(input.PAtm), fmt(input.TAtm)])
        .concat(resultColumns.map(() => '')).concat(['ERROR', errMsg]);
      rows.push(out);
      continue;
    }
    ...
    const out = inCols.map((k) => fmt(input[k]))
      .concat([fmt(input.PAtm), fmt(input.TAtm)])
      .concat(resVals)
      .concat([result.isValid ? '1' : '0', result.warning || '']);
    rows.push(out);
  }

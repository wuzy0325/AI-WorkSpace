...
- 监听端口：`127.0.0.1:18182`（与 daq-t1603 的 18181 区分）
- WebSocket 端点：`/ws`
- 事件清单（与主线 Wails 版一致）：
...
npm run build:backend        # Go 1.20.14 + npm build + go build → backend/daq-p1604-backend.exe
npm run dist:win7            # NSIS 打包 → dist/DAQ-P-1604-Win7-Setup-0.3.0-win7.1-x64.exe
```

### 与 daq-t1603 Win7 版的差异

- 端口 18182（daq-t1603 用 18181）
- 保留单体 App（daq-t1603 拆为 3 个 Service：device/log/recording），main.go 只调用一次 ServiceStartup/Shutdown

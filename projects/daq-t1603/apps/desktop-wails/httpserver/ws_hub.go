...
//   - Emit 非阻塞：broadcast channel 缓冲 128，超出即丢，避免 relay 协程 100ms
//     推送被阻塞；
...
//   - broadcast：128，吸收 relay 协程 100ms 推送的瞬时峰值。
func NewWSHub() *WSHub {
	return &WSHub{
...
// 行为约定：
//   - name 为空直接返回，避免误发空事件；
//   - data ...any 取首个元素作为 payload（与 backend.EmitEvent 调用约定一致），
//     无 data 时 payload 为 nil；

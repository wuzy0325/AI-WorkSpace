// Package httpserver log_handler：LogService 的 HTTP 包装。
//
// 路由约定：
...
func (s *Server) handleLogStart(w http.ResponseWriter, r *http.Request) {
	...
	if err := s.log.StartLogFile(body.OutputDir, body.Prefix); err != nil {
...
func (s *Server) handleLogStop(w http.ResponseWriter, r *http.Request) {
	...
	if err := s.log.StopLogFile(); err != nil {
		writeError(w, http.StatusInternalServerError, err.Error())
		return
	}
	writeOK(w, nil)
...
func (s *Server) handleLogState(w http.ResponseWriter, r *http.Request) {
	...
	writeOK(w, s.log.GetLogFileState())
}

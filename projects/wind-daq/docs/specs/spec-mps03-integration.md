# Spec: MPS-03 多功能探针设备集成到 wind-daq
...
将 MPS-03 多功能探针设备集成到 wind-daq 系统，作为新的 DAQ 设备类型。MPS-03 是基于 TCP/IP 的风洞测量数据采集设备（**非运动控制器**），输出 16 通道异构气动数据：攻角 α、侧滑角 β、马赫数 Ma、速度 V、三向速度 Vx/Vy/Vz、6 路传感器 S1-S6、总压 P_total、内外温度 T_ext/T_int。

### 与现有 DAQ-P-1603 的身份隔离（Critical）

wind-daq 已存在 **DAQ-P-1603**（`Type = "DAQ-P-1603"`，16 通道通用 AI 采集，DLL FFI 驱动）。MPS-03 与 DAQ-P-1603 协议、传输方式、数据语义完全不同，必须严格隔离：
...
1. MPS-03 可作为设备类型添加到 wind-daq，前后端 `Type = "MPS03"` 一致，与 DAQ-P-1603 完全隔离
...
# 跨模块集成测试（wind-daq API + shared SDK 模拟器）
cd projects/wind-daq/services/api-go
...
projects/wind-daq/services/api-go/internal/
  ├── core/device/types.go               ← wind-daq 自有领域类型
  ├── ports/device.go                    ← wind-daq 自有接口
  ├── adapters/hardware/mps03_adapter.go ← thin wrapper（翻译 shared ↔ wind-daq 类型）
  └── ...                                ← 其他 wind-daq 内部代码
```

**硬约束**：
- `shared/device-sdk/go/*` 禁止 `import "wind-daq/services/api-go/internal/*"`
- `shared/device-sdk/go/*` 只能依赖 `shared.local/device-sdk/go/*` 内部包
- wind-daq adapter 双向依赖：`wind-daq internal/*` + `shared.local/device-sdk/go/*`
...
| 8 | `projects/wind-daq/services/api-go/internal/adapters/hardware/mps03_adapter.go` | wind-daq thin wrapper（类型翻译） |
| 9 | `projects/wind-daq/apps/desktop-wails/frontend/src/components/device/Mps03ConfigPanel.vue` | MPS-03 配置面板组件 |
...
| 12 | `projects/wind-daq/services/api-go/internal/core/device/types.go` | 新增 `DeviceMPS03 Type = "MPS03"` + `MPS03HardwareConfig` + `Profile.MPS03Config`（wind-daq 自有类型，与 shared SDK 镜像） |
| 13 | `projects/wind-daq/services/api-go/internal/ports/device.go` | 新增 `MPS03Configurable` 接口 |
| 14 | `projects/wind-daq/services/api-go/internal/adapters/config/default_profiles.go` | `NewDefaultProfile` switch 新增 MPS03 分支（默认 IP/端口/16 通道）+ `NormalizeProfile` MPS03 规范化 |
| 15 | `projects/wind-daq/services/api-go/internal/bootstrap/bootstrap.go` | `deviceFactory.Create` switch 新增 `case device.DeviceMPS03` |
| 16 | `projects/wind-daq/services/api-go/api/server.go` | `handleDeviceByID` 新增 MPS03 配置端点 + `/api/storage/start` 收集 MPS03 channels |
| 17 | `projects/wind-daq/services/api-go/internal/adapters/storage/csv_sink.go` | `buildDynamicHeader` 新增 MPS03 分支（17 列固定表头） |
| 18 | `projects/wind-daq/apps/desktop-wails/frontend/src/api/types.ts` | `DeviceType` 联合新增 `'MPS03'` + `MPS03HardwareConfig` 接口 |
| 19 | `projects/wind-daq/apps/desktop-wails/frontend/src/api/deviceApi.ts` | 新增 `getMps03Config` / `applyMps03Config` |
| 20 | `projects/wind-daq/apps/desktop-wails/frontend/src/stores/deviceStore.ts` | 新增 MPS03 配置操作 + 校零跳过 T_ext/T_int |
| 21 | `projects/wind-daq/apps/desktop-wails/frontend/src/components/device/DeviceManagementDrawer.vue` | MPS03 类型分支（设备类型下拉、默认值、配置加载/保存、采样率只读展示、校零跳过） |
| 22 | `projects/wind-daq/apps/desktop-wails/backend/app.go` | 新增 Wails binding：`DeviceGetMps03Config` / `DeviceApplyMps03Config` |
...
### wind-daq thin wrapper 风格（参照 `daq_p1603_adapter.go`）

```go
// 文件：projects/wind-daq/services/api-go/internal/adapters/hardware/mps03_adapter.go
package hardware
...
// MPS03Adapter wind-daq 项目层 thin wrapper，委托给 shared SDK 的 MPS03Device。
// 仅做类型翻译：wind-daq Profile ↔ shared SDK Profile、Status、DataPayload。
...
- wind-daq HTTP 层将此错误映射为 `409 Conflict`
...
**Owner**：shared SDK 驱动层 `MPS03Device` 自身负责重连（不在 wind-daq adapter 或 DeviceManager 层）。
...
- 调用 `onError` 回调通知 wind-daq adapter
- wind-daq adapter 通知 DeviceManager 更新 UI 状态
...
```go
// 文件：projects/wind-daq/services/api-go/internal/core/device/types.go
// 与 shared SDK 镜像，但属于 wind-daq 自有类型，由 adapter 翻译
...
```go
// 文件：projects/wind-daq/apps/desktop-wails/backend/app.go
...
- UTF-8 with BOM（与 wind-daq 现有 CSV 一致，确保 Excel 正确识别中文路径）
- 行尾 `\r\n`（Windows 风格）
...
| wind-daq 适配器测试 | Go testing | `projects/wind-daq/.../adapters/hardware/mps03_adapter_test.go` | 类型翻译正确性、sink 转发、错误回调 |
| wind-daq API 集成测试 | Go testing | `projects/wind-daq/.../api/server_test.go`（新增 MPS03 用例） | HTTP 端点、409/400/502 状态码、采集期间禁止 Apply |
...
2. **shared SDK 依赖方向**：`shared/device-sdk/go/*` 禁止 `import "wind-daq/services/api-go/internal/*"`
...
12. **重连状态机内嵌**：重连由 shared SDK 驱动层负责，不在 wind-daq adapter 或 DeviceManager 层
...
16. **提交前验证**：shared SDK + wind-daq 两个模块均 `go build` + `go test` + `go vet` 全绿，前端 `npm typecheck` + `npm build` 全绿
...
1. 在 shared SDK 驱动层导入 `wind-daq/services/api-go/internal/*`
...
| 14 | wind-daq 模块 `go test ./...` + `go vet ./...` 全绿 | CI |
...
Phase 3: wind-daq 项目层（依赖 Phase 2）
...
Phase 5: Wails binding 同步 + 验证
  ...
  ├── 5.3 shared SDK + wind-daq 双模块 go test / go vet

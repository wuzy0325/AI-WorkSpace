...
6. **依赖方向**：`shared/device-sdk/go/*` 禁止 `import "wind-daq/services/api-go/internal/*"`
...
- [ ] `grep -r "wind-daq" shared/device-sdk/go/protocol/` 无命中
- [ ] 人工 review
...
- [ ] `grep -r "wind-daq" shared/device-sdk/go/` 无命中
...
- [ ] **T7** wind-daq core/ports 镜像
  - 依赖：T3
  - 文件：`projects/wind-daq/services/api-go/internal/core/device/types.go`（改）、`internal/ports/device.go`（改）
  - Verify：`cd projects/wind-daq/services/api-go && go build ./... && go vet ./...`
  - 关键点：wind-daq 自有 `DeviceMPS03` + `MPS03HardwareConfig` + `Profile.MPS03Config` + `ports.MPS03Configurable`（用 wind-daq 类型）

- [ ] **T8** MPS03Adapter thin wrapper
  - 依赖：T4, T7
  - 文件：`projects/wind-daq/services/api-go/internal/adapters/hardware/mps03_adapter.go`（新增）、`mps03_adapter_test.go`（新增可选）
  - Verify：`cd projects/wind-daq/services/api-go && go test ./internal/adapters/hardware/... -run MPS03 -v`
  - 关键点：编译期断言 + Profile/Status/DataPayload 双向翻译 + sink 包装 + onError 委托

- [ ] **T9** default_profiles + bootstrap 注册
  - 依赖：T7, T8
  - 文件：`projects/wind-daq/services/api-go/internal/adapters/config/default_profiles.go`（改）、`internal/bootstrap/bootstrap.go`（改）
  - Verify：`cd projects/wind-daq/services/api-go && go build ./... && go vet ./...`
...
- [ ] **T10** HTTP API 端点
  - 依赖：T8, T9
  - 文件：`projects/wind-daq/services/api-go/api/server.go`（改）
  - Verify：`cd projects/wind-daq/services/api-go && go test ./api/... -run MPS03 -v`
...
- [ ] **T11** CSV sink 17 列表头分支（与 T10 并行）
  - 依赖：T7
  - 文件：`projects/wind-daq/services/api-go/internal/adapters/storage/csv_sink.go`（改）
  - Verify：`cd projects/wind-daq/services/api-go && go test ./internal/adapters/storage/... -run MPS03 -v`
  - 关键点：`case device.DeviceMPS03:` + 17 列固定表头（Timestamp + 16 ASCII 列名）+ 禁用通道填空字符串 + UTF-8 BOM + `\r\n`

- [ ] **T12** API 集成测试
  - 依赖：T10, T11
  - 文件：`projects/wind-daq/services/api-go/api/server_test.go`（改）
  - Verify：`cd projects/wind-daq/services/api-go && go test ./api/... -run MPS03 -v`
...
- [ ] **T13** types.ts + deviceApi.ts
  - 依赖：T10
  - 文件：`projects/wind-daq/apps/desktop-wails/frontend/src/api/types.ts`（改）、`src/api/deviceApi.ts`（改）
  - Verify：`cd projects/wind-daq/apps/desktop-wails/frontend && npm run typecheck && npm run build`
  - 关键点：`DeviceType` 联合新增 `'MPS03'` + `MPS03HardwareConfig` 接口（9 字段）+ `getMps03Config`/`applyMps03Config` + 错误状态码处理

- [ ] **T14** Mps03ConfigPanel.vue
  - 依赖：T13
  - 文件：`projects/wind-daq/apps/desktop-wails/frontend/src/components/device/Mps03ConfigPanel.vue`（新增）
...
- [ ] **T15** DeviceManagementDrawer.vue MPS03 分支
  - 依赖：T13, T14
  - 文件：`projects/wind-daq/apps/desktop-wails/frontend/src/components/device/DeviceManagementDrawer.vue`（改）
  - Verify：`npm run typecheck && npm run build`
...
- [ ] **T16** deviceStore.ts MPS03 操作（与 T14/T15 并行）
  - 依赖：T13
  - 文件：`projects/wind-daq/apps/desktop-wails/frontend/src/stores/deviceStore.ts`（改）
  - Verify：`npm run typecheck && npm run build`
...
- [ ] **T17** Wails binding 新增 + 同步
  - 依赖：T10
  - 文件：`projects/wind-daq/apps/desktop-wails/backend/app.go`（改）+ 自动生成 TS binding
  - Verify：`cd projects/wind-daq/apps/desktop-wails && wails3 generate bindings -silent` + `go build ./... && go vet ./...` + `npm run typecheck && npm run build`

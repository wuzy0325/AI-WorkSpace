# Implementation Plan: wind-daq 代码审查问题整改
...
在 `projects/wind-daq/services/api-go` 下执行：
...
后端命令在 `projects/wind-daq/services/api-go` 下执行，前端命令在 `projects/wind-daq/apps/desktop-wails/frontend` 下执行：
...
在工作空间根目录执行结构校验，在 `projects/wind-daq/apps/desktop-wails/frontend` 下执行 npm 命令与 bundle 扫描：
...
- `go vet ./...`
- `rg -n "log\.Printf" .\projects\wind-daq\services\api-go\internal\usecase\calibration.go` returns no matches.

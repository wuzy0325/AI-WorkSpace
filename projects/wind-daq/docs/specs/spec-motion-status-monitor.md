...
1. 本规格先覆盖 B140、WTNMC4A 与模拟运动控制器，同时服务 wind-daq 与独立 motion-controller 项目。
...
- 不在本规格中把 wind-daq 的运动安全策略全部迁入 shared；是否提升另立决策。
...
### wind-daq Backend Port

wind-daq 的安全用例不能继续只依赖 `StatusAll(ctx) []ControllerStatus`，否则无法判断 sequence、generation、失败和 freshness。项目侧新增只读端口，adapter 将 shared monitor 快照转换为项目类型：
...
- 提交前运行 shared、wind-daq 后端和前端定向/全量验证。
...
4. **迁移范围**：本轮是否同时改造独立 motion-controller 项目，还是先在 shared + wind-daq 落地，再单独迁移 motion-controller？推荐同一规格覆盖，按任务分批实现。

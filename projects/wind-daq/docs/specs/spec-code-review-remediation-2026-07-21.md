...
6. 本规格、后续计划和任务文档均保存在 `projects/wind-daq/docs/specs/`；规格获批前不得进入计划、任务或实现阶段。

## Objective

整改 2026-07-21 静态代码审查确认的问题，使 wind-daq 的 HTTP API、usecase、adapter、Vue 前端和异步生命周期重新符合工作空间六边形架构、前后端职责边界和可观测性规则，同时不破坏现有设备、校准与遍历工作流。
...
powershell -NoProfile -ExecutionPolicy Bypass -File .\scripts\validate-frontend-structure.ps1 -ProjectDir .\projects\wind-daq\apps\desktop-wails\frontend\src -CheckFileSize

# 后端格式、构建、静态检查和测试
Push-Location .\projects\wind-daq\services\api-go
gofmt -l .
...
# 前端类型、单测和生产构建
Push-Location .\projects\wind-daq\apps\desktop-wails\frontend
npm run typecheck
npm run test
npm run build
Pop-Location
...
Push-Location .\projects\wind-daq\apps\desktop-wails
...
- [ ] 所有适用的结构、Go、前端验证命令退出码为 0；任何未执行项在报告中明确标记原因和风险。**【Blocked / Waiver】**
  ...
  - **未通过项（waiver/blocker，预存工作区问题，非本轮整改回归）**：
    - `validate-structure.ps1` → exit 1（12 个未知顶层条目：`.trae`/`.codebuddy`/`.workbuddy`/`.worktrees`/`analysis`/`pdf-qa`/`temp`/`wind-daq-intro-images`/`wind-daq-trae-work-prototype`/`.tmp-protocol-remaining.html`/`multi-probe-traversal-design.html`/`W505-Protocol-V2.0.html`）。

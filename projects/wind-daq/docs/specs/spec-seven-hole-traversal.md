...
**构建注册**：`go.work` 的 `use` 列表加入 `./shared/algorithms/go/sevenhole`；`projects/wind-daq/services/api-go/go.mod` 加入 require + `replace ai-workspace/shared/algorithms/go/sevenhole => ../../../../shared/algorithms/go/sevenhole`（与 fivehole L6/L13 同款）。
...
1. 数据集 `projects/wind-daq/docs/W532.202608.P.7H.1-01/` 的 7 个 CSV 为 GBK 编码且表头存在历史命名错误（spec-seven-hole-calibration.md §12.1），**按列位置读取**并转码 UTF-8 副本（不改原始文件）。
...
- api-go 全量：`cd projects\wind-daq\services\api-go; go test ./...`（含 usecase 既有 `TestBuildRawPressure_*`、`TestParseConfig`、遍历 v2 集成测试）
- 前端：`cd projects\wind-daq\apps\desktop-wails\frontend; npm run typecheck; npm run build`
...
| 模块注册方式 | `go.work` 的 `use` 列表；`projects/wind-daq/services/api-go/go.mod` 的 require/replace |

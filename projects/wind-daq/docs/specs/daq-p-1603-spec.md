# Spec: wind-daq 新增 DAQ-P-1603 压力采集设备

## Objective

在 `projects/wind-daq` 多设备 DAQ 应用中新增 **DAQ-P-1603** 压力采集设备支持，使其与现有 DAQ-P-1604 / DAQ-T-1603 / DSA3217 / WTN_PXI 并列，可在统一 UI 中连接、配置、采集、录制。

**用户故事**：
- 作为风洞测试工程师，我希望在 wind-daq 中连接 DAQ-P-1603 设备，配置 16 通道压力采集参数，实时查看压力曲线，并将数据录制为 CSV。
...
| 前端 | Vue 3 + TypeScript + Vite + Naive UI + Tailwind | 与 wind-daq 现有一致 |
...
```powershell
# 工作区根目录执行
$env:GOWORK="off"   # 仅 daq-t1603 排除场景需要；wind-daq 默认走 go.work
...
# 2. wind-daq 后端编译验证
cd c:\Users\wuzhy\Documents\D\SVN\SoftWare\trunk\AI-Workspace\projects\wind-daq\services\api-go
go build ./...
go vet ./...
go test ./...

# 3. wind-daq Wails 应用构建
cd c:\Users\wuzhy\Documents\D\SVN\SoftWare\trunk\AI-Workspace\projects\wind-daq\apps\desktop-wails
go build ./...
go vet ./...

# 4. 前端类型检查 + 构建
cd c:\Users\wuzhy\Documents\D\SVN\SoftWare\trunk\AI-Workspace\projects\wind-daq\apps\desktop-wails\frontend
npm run typecheck
npm run build
...
├── adapters/hardware/
│   └── daq_p1603.go           【新增】thin wrapper，桥接 shared SDK 与 wind-daq ports
└── bootstrap/
...
| `projects/wind-daq/.../adapters/hardware/` | thin wrapper | 仅做接口桥接，零硬件直调 |
| `projects/wind-daq/.../bootstrap/` | 装配根 | 工厂 switch，零业务逻辑 |
| `projects/wind-daq/.../frontend/` | UI | 零硬件访问，零采集算法 |
...
| 前端组件测试 | （wind-daq 现有项目无单测框架，省略） | — | — |
| 手工 HIL 测试 | 人工执行 | `projects/wind-daq/docs/runbooks/hil-validation-plan.md` | 真机连接、采集、断网恢复 |
...
- wind-daq thin wrapper：≥ 60%（thin wrapper 本身逻辑少）
...
| # | 条件 | 验证方式 |
|---|---|---|
| 1 | wind-daq 后端 `go build ./... && go vet ./... && go test ./...` 全绿 | 命令行执行 |
...
1. **DLL 部署路径**：`WTNDAQ16H_64.dll` 放在 `projects/wind-daq/apps/desktop-wails/` 根目录还是 `third_party/` 子目录？是否需要 go:embed？（建议：build 时复制到可执行文件同目录，不 embed）
2. **单位换算**：DAQ-P-1603 输出原始 ADC 码值（U16），如何换算为压力/温度单位？是否需要校准系数表？厂商是否提供 `ScaleBinToVolt` 等同的换算 API？温度通道的换算公式是否与压力不同？
3. **触发模式**：示例代码同时启用了数字触发（DIO1 下降沿）和模拟触发。wind-daq 默认场景是否需要触发？还是默认软件触发、UI 暴露触发配置？

...
**Verification:**
- [ ] 在 `projects\wind-daq\services\api-go` 执行 `go build -buildvcs=false ./...`
...
- [ ] 在 `projects\wind-daq\services\api-go` 执行 `go test -race ./internal/usecase`
...
**Acceptance criteria:**
- [ ] `mode` 状态持久化到 localStorage（`wind-daq.traversal.mode`）

...
func main() {
	...
	rate := flag.Float64("rate", 500, "采样率 (Hz, 默认 500，对齐 wind-daq DAQP1603MaxSampleRate)")
	useCurrent := flag.Bool("current", true, "使用 0-20mA 电流模式（默认开启，对齐 wind-daq）")
	dllPath := flag.String("dll", "", "WTNDAQ16H_64.dll 路径")
	continuous := flag.Duration("continuous", 0, "持续采集时长（如 30s, 1m），0 表示仅读 3 次。模拟 wind-daq readLoop 行为")
	nChans := flag.Int("chans", 16, "启用通道数（1-16）。wind-daq 默认按 profile 配置，本工具默认 16 通道")
	winddaqMode := flag.Bool("winddaq", false, "完全复刻 wind-daq 调用路径（含 GetVoltRangeInfo + ScaleBinToVolt）")
	applyConfigMode := flag.Bool("applyconfig", false, "复刻 wind-daq ApplyConfig 路径：Connect 后再 ReleaseTask→VerifyParam→InitTask")
	multiThreadMode := flag.Bool("multithread", false, "复刻 wind-daq 多 goroutine 调用：Connect/InitTask/StartTask 在主 goroutine，ReadBinary 在新 goroutine（验证 OS 线程亲和性假设）")
	flag.Parse()
	...
	// Step 3: InitTask（按通道数构造参数，对齐 wind-daq buildAIParamLocked）
	...
	// ApplyConfig 复刻模式：模拟 wind-daq 在 Connect 后再次调用 ApplyConfig
	...
	// Step 4: StartTask + SendSoftTrig（完全复刻 wind-daq driver startAndCheck）
	fmt.Printf("[4/5] 启动采集 (复刻 wind-daq startAndCheck) ... ")
	...
	// 诊断点 A：StartTask 后立即查 TaskState（对齐 wind-daq driver）
	...
	// 诊断点 B：SendSoftTrig 后查 TaskState（对齐 wind-daq driver）
	...
	// readLoop 启动前 GetStatus（完全对齐 wind-daq driver readLoop 第 546-557 行）
	...
	// wind-daq 复刻模式：完全对齐 wind-daq readLoop 的换算逻辑（Go 端 U16→mA→工程量）
	currentBuf := make([]float64, *nChans)
	if *winddaqMode {
		fmt.Printf("[wind-daq 复刻] 使用 Go 端换算（U16 → mA → 工程量）\n\n")
	}
	...
	if *multiThreadMode && *continuous > 0 {
		// 多线程复刻模式：ReadBinary 在新 goroutine 执行，复刻 wind-daq driver readLoop goroutine。
...
// buildAIParam 按 wind-daq buildAIParamLocked 的逻辑构造 AI 参数
// 差异：本工具按 -chans 参数启用前 N 个通道，wind-daq 按 profile.Channels[].Enabled

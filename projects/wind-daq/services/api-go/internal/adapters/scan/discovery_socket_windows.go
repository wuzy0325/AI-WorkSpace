...
func (s *winsockDiscoverySocket) Send(data []byte, target string, port int) error {
	...
	sendTimeoutMs := int(discoverySendTimeout.Milliseconds())
	if sendTimeoutMs < 1 {
		sendTimeoutMs = 1
	}
	if err := windows.SetsockoptInt(handle, windows.SOL_SOCKET, soSNDTIMEO, sendTimeoutMs); err != nil {
		return err
	}
...
func (s *winsockDiscoverySocket) Receive(buf []byte, timeout time.Duration) (int, string, error) {
	...
	recvTimeoutMs := int(timeout.Milliseconds())
	if recvTimeoutMs < 1 {
		recvTimeoutMs = 1
	}
	if err := windows.SetsockoptInt(handle, windows.SOL_SOCKET, windows.SO_RCVTIMEO, recvTimeoutMs); err != nil {
		return 0, "", err
	}

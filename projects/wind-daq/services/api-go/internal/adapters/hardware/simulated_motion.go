...
type SimulatedMotionController struct {
	...
	cancelChs map[motion.AxisName]chan struct{}
}
...
func NewSimulatedMotionController(profile motion.MotionControllerProfile) *SimulatedMotionController {
	...
	cancelChs := make(map[motion.AxisName]chan struct{})
...
// ResetEmergencyStop 复位紧急停止状态
func (c *SimulatedMotionController) ResetEmergencyStop() error {
	c.mu.Lock()
	defer c.mu.Unlock()
...
func (c *SimulatedMotionController) cancelAxisLocked(axis motion.AxisName) {
...
func (c *SimulatedMotionController) axisProfileLocked(axis motion.AxisName) (motion.AxisConfig, bool) {
	for _, a := range c.profile.Axes {
		if a.Name == axis {
			return a, true
		}
	}
	return motion.AxisConfig{}, false
}
...
func (c *SimulatedMotionController) axisNameByIndexLocked(axisIndex int) motion.AxisName {
...
func (c *SimulatedMotionController) simulateMovement(axisIndex int, axis motion.AxisName, targetPosition, direction, speed float64, duration time.Duration, stopCh, cancelCh chan struct{}) {
...
// simulateJog 模拟持续点动
func (c *SimulatedMotionController) simulateJog(axisIndex int, axis motion.AxisName, velocity float64, maxSpeed float64, stopCh, cancelCh chan struct{}) {
	ticker := time.NewTicker(20 * time.Millisecond)
	defer ticker.Stop()
...
// simulateHome 模拟归位过程
func (c *SimulatedMotionController) simulateHome(axisIndex int, axis motion.AxisName, stopCh, cancelCh chan struct{}) {
	ticker := time.NewTicker(20 * time.Millisecond)
	defer ticker.Stop()

...
// ------------------------------------------------------------
// 职责：在 wind-daq 项目类型（device.Profile/Status/DataPayload）与
...
func mapToSharedProfileP1603(p device.Profile) sharedcore.Profile {
	...
	for i, ch := range p.Channels {
		...
		// 若用户在前端把通道改为 temperature 但 wind-daq ChannelConfig.SensorType
...
// mapFromSharedProfileP1603 将 shared SDK 的 Profile 转回 wind-daq 的 Profile。
// fallback 用于补全 wind-daq 端独有字段（如 SerialPort、BaudRate、AutoConnect 等），
...
// mapToDeviceStatusP1603 将 shared SDK 的 Status 转为 wind-daq 的 Status。
...
// mapToDevicePayloadP1603 将 shared SDK 的 DataPayload 转为 wind-daq 的 DataPayload。
// DeviceType/DeviceName 由 adapter 注入（driver 不知道 wind-daq 项目层的设备名）。

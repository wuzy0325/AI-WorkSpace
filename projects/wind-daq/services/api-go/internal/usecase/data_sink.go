...
//   2. hub.OnData：同步更新内存索引（分片锁，<1μs）
//   3. recorder.HandlePayload：内部 sink.Write 已异步化（投递到 channel 立即返回）
// 整体不阻塞设备 read loop，支撑 1kHz × 10 设备全量保存。
func NewDataSink(hub *AcquisitionHub, recorder *StorageRecorder) func(device.DataPayload) {

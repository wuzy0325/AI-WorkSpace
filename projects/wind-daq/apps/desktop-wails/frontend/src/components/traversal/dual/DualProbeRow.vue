...
// 已等待时长：后端 waitingForAcquisitionSinceMs 单一时钟源；本地 1s ticker 刷新，
// 不依赖 status 轮询——since 字段不变时 polling 去重不会持续触发 computed 更新。
// 组件卸载时清理 timer。
...
const waitingDurationText = computed(() => {
  const since = session.value.status?.waitingForAcquisitionSinceMs
  if (typeof since !== 'number' || since <= 0) return ''
  const secs = Math.max(0, Math.floor((nowMs.value - since) / 1000))

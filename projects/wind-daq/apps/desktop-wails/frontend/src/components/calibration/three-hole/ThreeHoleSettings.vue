...
const pointLayout = ref<ThreeHolePointLayout>({
  thetaMin: -30,
  thetaMax: 30,
  thetaStep: 5
})
...
const pointCount = computed(() => {
  return Math.floor((pointLayout.value.thetaMax - pointLayout.value.thetaMin) / pointLayout.value.thetaStep) + 1
})
...
const probeChannels = ref<ProbeChannelConfig[]>([
  { name: '左孔压力', role: 'threeHole.p1', channel: { deviceId: '', channelIndex: 0 }, enabled: true, precision: DEFAULT_CALIBRATION_PROBE_PRECISION },
  { name: '中孔压力（中心孔）', role: 'threeHole.p2', channel: { deviceId: '', channelIndex: 1 }, enabled: true, precision: DEFAULT_CALIBRATION_PROBE_PRECISION },
  { name: '右孔压力', role: 'threeHole.p3', channel: { deviceId: '', channelIndex: 2 }, enabled: true, precision: DEFAULT_CALIBRATION_PROBE_PRECISION },
  { name: '大气压力', role: 'threeHole.pAtm', channel: { deviceId: '', channelIndex: 16 }, enabled: true, precision: DEFAULT_CALIBRATION_PROBE_PRECISION },
  { name: '总压（可选）', role: 'threeHole.pTotal', channel: { deviceId: '', channelIndex: -1 }, enabled: false, precision: DEFAULT_CALIBRATION_PROBE_PRECISION }
])

const motionAxes = ref<MotionAxisConfig[]>([
  { name: 'θ', controllerId: '', axis: 'X' }
])
...
const currentStepErrors = computed<string[]>(() => {
  if (currentStep.value === 0) {
    ...
    if (pointLayout.value.thetaStep <= 0) errors.push('偏角步长必须大于 0')
    if (pointLayout.value.thetaMax <= pointLayout.value.thetaMin) errors.push('偏角最大值必须大于最小值')
    const thetaRange = pointLayout.value.thetaMax - pointLayout.value.thetaMin
    if (thetaRange % pointLayout.value.thetaStep !== 0) errors.push('偏角范围需能被步长整除')
  ...
  if (currentStep.value === 1) {
    ...
    if (missingRoles.length > 0) errors.push('左/中/右孔与大气压力必须启用并绑定')
...
function generatePoints() {
  const points = []
  let id = 0
  for (let theta = pointLayout.value.thetaMin; theta <= pointLayout.value.thetaMax; theta += pointLayout.value.thetaStep) {
    points.push({ id: id++, coordinates: { θ: theta } })
  }
  return points
}
...
async function saveConfig() {
  ...
  try {
    ...
    const res = await calibrationApi.saveConfig('three-hole', JSON.parse(JSON.stringify(normalizedConfig)))
    if (!res.success) {
...
<template>
  <div class="fixed inset-0 z-50 flex items-center justify-center bg-slate-950/35 backdrop-blur-sm">
    <div data-test="three-hole-settings-shell" class="flex max-h-[90vh] w-[92vw] max-w-[980px] flex-col overflow-hidden rounded-[var(--radius-lg)] border border-[var(--border-default)] bg-[var(--bg-panel)] text-[var(--text-primary)] shadow-[0_24px_80px_rgba(15,23,42,0.16)]">
      <div class="flex items-center justify-between border-b border-[var(--border-default)] px-6 py-4">
        <div>
          ...
          <h1 class="text-xl font-bold text-[var(--text-primary)]">三孔探针校准配置</h1>
      ...
      <div data-test="three-hole-settings-stepper" class="border-b border-[var(--border-default)] bg-[var(--bg-panel-strong)] px-6 py-3">
      ...
      <div class="flex-1 overflow-auto p-6">
        ...
        <div v-else-if="currentStep === 0" class="max-w-3xl mx-auto space-y-6">
          ...
          <div class="bg-[var(--bg-panel)] border border-[var(--border-default)] rounded-[var(--radius-md)] p-6">
            ...
            <div class="space-y-4">
              <div>
                <label class="block text-sm text-[var(--text-secondary)] mb-2">偏角θ范围</label>
                <div class="grid grid-cols-3 gap-4">
                  <div>
                    <label class="block text-xs text-[var(--text-muted)] mb-1">最小值 (°)</label>
                    <input v-model.number="pointLayout.thetaMin" type="number" step="1" class="w-full px-4 py-2 bg-[var(--bg-panel-strong)] border border-[var(--border-default)] rounded-[var(--radius-sm)] text-[var(--text-primary)] focus:border-[var(--accent-primary)] focus:outline-none transition-colors" />
                  </div>
                  <div>
                    <label class="block text-xs text-[var(--text-muted)] mb-1">最大值 (°)</label>
                    <input v-model.number="pointLayout.thetaMax" type="number" step="1" class="w-full px-4 py-2 bg-[var(--bg-panel-strong)] border border-[var(--border-default)] rounded-[var(--radius-sm)] text-[var(--text-primary)] focus:border-[var(--accent-primary)] focus:outline-none transition-colors" />
                  </div>
                  <div>
                    <label class="block text-xs text-[var(--text-muted)] mb-1">步长 (°)</label>
                    <input v-model.number="pointLayout.thetaStep" type="number" min="1" step="1" class="w-full px-4 py-2 bg-[var(--bg-panel-strong)] border border-[var(--border-default)] rounded-[var(--radius-sm)] text-[var(--text-primary)] focus:border-[var(--accent-primary)] focus:outline-none transition-colors" />
                  </div>
                </div>
              </div>
            </div>
          ...
          <div class="bg-[var(--bg-panel)] border border-[var(--border-default)] rounded-[var(--radius-md)] p-6">
            ...
            <div class="space-y-3 text-sm">
              ...
              <div class="flex justify-between py-2 border-b border-[var(--border-default)]">
                ...
                <span class="text-[var(--text-primary)]">三孔探针</span>
              </div>
              <div class="flex justify-between py-2 border-b border-[var(--border-default)]">
                <span class="text-[var(--text-secondary)]">点位布局</span>
                <span class="text-[var(--text-primary)]">θ: {{ pointLayout.thetaMin }}° ~ {{ pointLayout.thetaMax }}° (步长 {{ pointLayout.thetaStep }}°)</span>
              </div>
              <div class="flex justify-between py-2 border-b border-[var(--border-default)]">
          ...
          <div class="bg-[var(--accent-primary)]/10 border border-[var(--accent-primary)]/30 rounded-[var(--radius-md)] p-4">
            <div class="flex items-start gap-3">
              ...
              <div class="text-sm">
                ...
                <p class="mt-1 text-[var(--text-secondary)]">保存后，配置将自动存储。下次进入三孔探针校准时，将自动加载此配置。</p>
              </div>
            </div>
          </div>
        </div>

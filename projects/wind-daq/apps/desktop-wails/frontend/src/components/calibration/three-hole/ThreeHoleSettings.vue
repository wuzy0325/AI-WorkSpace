...
<template>
  <NModal :show="true" preset="card" :style="{ maxWidth: '980px', width: '92vw' }" title="三孔探针校准配置" closable @close="emit('close')">
    ...
    <template v-else>
      ...
      <div v-if="currentStep === 0" class="step-content">
        ...
        <NCard size="small" :bordered="true" class="section-card">
          ...
          <NText depth="3" style="font-size:11px;display:block;margin-bottom:8px">角度 θ 范围</NText>
          <div class="mach-grid">
            <div><NText depth="3" style="font-size:11px">最小值 (°)</NText><NInputNumber v-model:value="pointLayout.thetaMin" size="small" style="width:100%" /></div>
            <div><NText depth="3" style="font-size:11px">最大值 (°)</NText><NInputNumber v-model:value="pointLayout.thetaMax" size="small" style="width:100%" /></div>
            <div><NText depth="3" style="font-size:11px">步长 (°)</NText><NInputNumber v-model:value="pointLayout.thetaStep" :min="1" size="small" style="width:100%" /></div>
          </div>
      ...
      <div v-if="currentStep === 2" class="step-content">
        <NCard size="small" :bordered="true" class="section-card">
          ...
          <div class="summary-grid"><div class="summary-row"><NText depth="3">配置名称</NText><NText depth="1">{{ calibrationName }}</NText></div><div class="summary-row"><NText depth="3">校准类型</NText><NText depth="1">三孔探针</NText></div><div class="summary-row"><NText depth="3">点位布局</NText><NText depth="1">θ: {{ pointLayout.thetaMin }}° ~ {{ pointLayout.thetaMax }}°（步长 {{ pointLayout.thetaStep }}°）</NText></div><div class="summary-row"><NText depth="3">总点数</NText><NText depth="1" style="color:var(--accent-primary);font-weight:700">{{ pointCount }} 点</NText></div><div class="summary-row"><NText depth="3">启用测点</NText><NText depth="1">{{ probeChannels.filter(ch => ch.enabled).length }} 个</NText></div><div class="summary-row"><NText depth="3">稳定时间</NText><NText depth="1">{{ dwellTimeMs }} ms</NText></div><div class="summary-row"><NText depth="3">每点采样</NText><NText depth="1">{{ samplesPerPoint }} 次</NText></div></div>
        </NCard>
      </div>
    </template>

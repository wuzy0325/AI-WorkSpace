...
const pointLayout = ref<TotalPressurePointLayout>({
  alphaMin: -30,
  alphaMax: 30,
  alphaStep: 5
})
...
const currentStepErrors = computed<string[]>(() => {
  if (currentStep.value === 0) {
    ...
    if (pointLayout.value.alphaStep <= 0) errors.push('攻角步长必须大于 0')
    if (pointLayout.value.alphaMax <= pointLayout.value.alphaMin) errors.push('攻角最大值必须大于最小值')
    const alphaRange = pointLayout.value.alphaMax - pointLayout.value.alphaMin
    if (alphaRange % pointLayout.value.alphaStep !== 0) errors.push('攻角范围需能被步长整除')
  ...
  if (currentStep.value === 1) {
    ...
    if (missingRoles.length > 0) errors.push('总压/静压/大气压必须启用并绑定')
...
function generatePoints() {
  const points = []
  let id = 0
  for (let alpha = pointLayout.value.alphaMin; alpha <= pointLayout.value.alphaMax; alpha += pointLayout.value.alphaStep) {
    points.push({ id: id++, coordinates: { α: alpha } })
  }
  return points
}
...
<template>
  <div class="fixed inset-0 z-50 flex items-center justify-center bg-slate-950/35 backdrop-blur-sm">
    <div data-test="total-pressure-settings-shell" class="flex max-h-[90vh] w-[92vw] max-w-[980px] flex-col overflow-hidden rounded-[var(--radius-lg)] border border-[var(--border-default)] bg-[var(--bg-panel)] text-[var(--text-primary)] shadow-[0_24px_80px_rgba(15,23,42,0.16)]">
      <div class="flex items-center justify-between border-b border-[var(--border-default)] px-6 py-4">
        <div>
          ...
          <h1 class="text-xl font-bold text-[var(--text-primary)]">总压探针校准配置</h1>
      ...
      <div data-test="total-pressure-settings-stepper" class="border-b border-[var(--border-default)] bg-[var(--bg-panel-strong)] px-6 py-3">
      ...
      <div class="flex-1 overflow-auto p-6">
        ...
        <div v-else-if="currentStep === 0" class="max-w-3xl mx-auto space-y-6">
          ...
          <div class="bg-[var(--bg-panel)] border border-[var(--border-default)] rounded-[var(--radius-md)] p-6">
            ...
            <div class="space-y-4">
              <div>
                <label class="block text-sm text-[var(--text-secondary)] mb-2">攻角α范围</label>
                <div class="grid grid-cols-3 gap-4">
                  <div>
                    <label class="block text-xs text-[var(--text-muted)] mb-1">最小值 (°)</label>
                    <input v-model.number="pointLayout.alphaMin" type="number" step="1" class="w-full px-4 py-2 bg-[var(--bg-panel-strong)] border border-[var(--border-default)] rounded-[var(--radius-sm)] text-[var(--text-primary)] focus:border-[var(--accent-primary)] focus:outline-none transition-colors" />
                  </div>
                  <div>
                    <label class="block text-xs text-[var(--text-muted)] mb-1">最大值 (°)</label>
                    <input v-model.number="pointLayout.alphaMax" type="number" step="1" class="w-full px-4 py-2 bg-[var(--bg-panel-strong)] border border-[var(--border-default)] rounded-[var(--radius-sm)] text-[var(--text-primary)] focus:border-[var(--accent-primary)] focus:outline-none transition-colors" />
                  </div>
                  <div>
                    <label class="block text-xs text-[var(--text-muted)] mb-1">步长 (°)</label>
                    <input v-model.number="pointLayout.alphaStep" type="number" min="1" step="1" class="w-full px-4 py-2 bg-[var(--bg-panel-strong)] border border-[var(--border-default)] rounded-[var(--radius-sm)] text-[var(--text-primary)] focus:border-[var(--accent-primary)] focus:outline-none transition-colors" />
                  </div>
                </div>
              </div>
            </div>
          ...
          <div class="bg-[var(--bg-panel)] border border-[var(--border-default)] rounded-[var(--radius-md)] p-6">
            ...
            <div class="space-y-3 text-sm">
              ...
              <div class="flex justify-between py-2 border-b border-[var(--border-default)]">
                ...
                <span class="text-[var(--text-primary)]">总压探针</span>
              </div>
              <div class="flex justify-between py-2 border-b border-[var(--border-default)]">
                <span class="text-[var(--text-secondary)]">点位布局</span>
                <span class="text-[var(--text-primary)]">α: {{ pointLayout.alphaMin }}° ~ {{ pointLayout.alphaMax }}° (步长 {{ pointLayout.alphaStep }}°)</span>
              </div>
              <div class="flex justify-between py-2 border-b border-[var(--border-default)]">
          ...
          <div class="bg-[var(--accent-primary)]/10 border border-[var(--accent-primary)]/30 rounded-[var(--radius-md)] p-4">
            <div class="flex items-start gap-3">
              ...
              <div class="text-sm">
                ...
                <p class="mt-1 text-[var(--text-secondary)]">保存后，配置将自动存储。下次进入总压探针校准时，将自动加载此配置。</p>
              </div>
            </div>
          </div>
        </div>

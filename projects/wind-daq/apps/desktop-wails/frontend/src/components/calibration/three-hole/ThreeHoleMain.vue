...
import type { CalibrationConfig, ThreeHoleDataPoint, ThreeHoleCoefficients } from '@shared/types/calibration'
...
const showCoefficientPanel = ref(true)
const activeTab = ref<'overview' | 'chart' | 'data'>('overview')
const kThetaCanvas = ref<HTMLCanvasElement | null>(null)
const cvThetaCanvas = ref<HTMLCanvasElement | null>(null)
...
const currentTheta = computed(() => {
  const coords = progressInfo.value?.currentPoint
  return coords && typeof coords.θ === 'number' ? coords.θ : null
})
...
function drawChart(canvasRef: typeof kThetaCanvas, xLabel: string, yLabel: string, xKey: keyof ThreeHoleCoefficients, yKey: keyof ThreeHoleCoefficients) {
  ...
  const dataPoints = calibrationStore.dataPoints
    .filter((p): p is ThreeHoleDataPoint => isThreeHoleDataPoint(p))
    .map((p) => ({ x: p.coefficients[xKey], y: p.coefficients[yKey] }))
...
function startChartUpdate() {
  ...
  chartUpdateTimer.value = setInterval(() => {
    if (activeTab.value === 'chart') {
      drawChart(kThetaCanvas, 'θ', 'K', 'K', 'K')
      drawChart(cvThetaCanvas, 'θ', 'Cv', 'K', 'Cv')
    }
    ...
    <div class="flex flex-1 overflow-hidden">
      ...
      <div class="flex w-80 flex-col border-r border-[var(--border-default)] bg-[var(--bg-panel)] overflow-y-auto flex-shrink-0">
        ...
        <div class="border-b border-[var(--border-default)] p-4">
          ...
          <div v-if="currentTheta !== null" class="rounded-lg bg-[var(--bg-panel-strong)] p-3 text-center">
            <div class="text-xs text-[var(--text-muted)]">偏航角 θ</div>
            <div class="font-mono text-lg font-bold text-[var(--accent-primary)]">{{ currentTheta.toFixed(1) }}°</div>
          </div>
      ...
      <div class="flex flex-1 flex-col min-w-0">
            ...
            <div class="rounded-xl border border-[var(--border-default)] bg-[var(--bg-panel)] p-5 shadow-[var(--shadow-panel)]">
              ...
              <div v-if="latestCoefficients" class="grid grid-cols-2 gap-3">
                <div class="rounded-lg bg-[var(--bg-panel-strong)] p-3">
                  <div class="text-xs text-[var(--text-muted)]">K</div>
                  <div class="font-mono text-lg font-bold text-[var(--accent-primary)]">{{ formatValue(latestCoefficients.K, 4) }}</div>
                </div>
                <div class="rounded-lg bg-[var(--bg-panel-strong)] p-3">
                  <div class="text-xs text-[var(--text-muted)]">Cv</div>
                  <div class="font-mono text-lg font-bold text-[var(--accent-primary)]">{{ formatValue(latestCoefficients.Cv, 4) }}</div>
                </div>
                <div class="rounded-lg bg-[var(--bg-panel-strong)] p-3">
                  <div class="text-xs text-[var(--text-muted)]">Cp</div>
                  <div class="font-mono text-lg font-bold text-[var(--accent-primary)]">{{ formatValue(latestCoefficients.Cp, 4) }}</div>
                </div>
              </div>
            ...
            <div class="rounded-xl border border-[var(--border-default)] bg-[var(--bg-panel)] p-5 shadow-[var(--shadow-panel)]">
              ...
              <div v-if="latestRawData" class="space-y-2">
                <div class="flex justify-between rounded-lg bg-[var(--bg-panel-strong)] px-3 py-2">
                  <span class="text-xs text-[var(--text-muted)]">P1 (左孔)</span>
                  <span class="font-mono text-sm font-bold text-[var(--text-primary)]">{{ formatValue(latestRawData.p1, 3) }} Pa</span>
                </div>
                <div class="flex justify-between rounded-lg bg-[var(--bg-panel-strong)] px-3 py-2">
                  <span class="text-xs text-[var(--text-muted)]">P2 (中孔)</span>
                  <span class="font-mono text-sm font-bold text-[var(--text-primary)]">{{ formatValue(latestRawData.p2, 3) }} Pa</span>
                </div>
                <div class="flex justify-between rounded-lg bg-[var(--bg-panel-strong)] px-3 py-2">
                  <span class="text-xs text-[var(--text-muted)]">P3 (右孔)</span>
                  <span class="font-mono text-sm font-bold text-[var(--text-primary)]">{{ formatValue(latestRawData.p3, 3) }} Pa</span>
                </div>
                <div class="flex justify-between rounded-lg bg-[var(--bg-panel-strong)] px-3 py-2">
              ...
              <div class="grid grid-cols-4 gap-4 text-sm">
                ...
                <div>
                  ...
                  <div class="font-medium text-[var(--text-primary)]">
                    <span v-if="threeHoleLayout">
                      θ: {{ threeHoleLayout.thetaMin }}°~{{ threeHoleLayout.thetaMax }}° ({{ threeHoleLayout.thetaStep }}°)
                    </span>
        ...
        <div v-if="activeTab === 'chart'" class="flex-1 overflow-hidden p-5">
          <div class="grid h-full grid-cols-2 gap-4">
            <div class="rounded-xl border border-[var(--border-default)] bg-[var(--bg-panel)] p-5 shadow-[var(--shadow-panel)]">
              <h3 class="mb-4 text-base font-semibold text-[var(--text-primary)]">K-θ 曲线</h3>
              <canvas ref="kThetaCanvas" class="h-[calc(100%-3rem)] w-full"></canvas>
            </div>
            <div class="rounded-xl border border-[var(--border-default)] bg-[var(--bg-panel)] p-5 shadow-[var(--shadow-panel)]">
              <h3 class="mb-4 text-base font-semibold text-[var(--text-primary)]">Cv-θ 曲线</h3>
              <canvas ref="cvThetaCanvas" class="h-[calc(100%-3rem)] w-full"></canvas>
            </div>
          </div>
        </div>
              ...
              <table class="w-full text-sm">
                <thead class="bg-[var(--bg-panel-strong)]">
                  <tr>
                    ...
                    <th class="px-3 py-2 text-left text-xs font-medium text-[var(--text-muted)]">Cv</th>
                    <th class="px-3 py-2 text-left text-xs font-medium text-[var(--text-muted)]">Cp</th>
                ...
                <tbody class="divide-y divide-[var(--border-default)]">
                  <tr v-for="(point, index) in calibrationStore.dataPoints" :key="index" class="hover:bg-[var(--bg-panel-strong)]">
                    ...
                    <td class="px-3 py-2 font-mono text-[var(--text-primary)]">{{ isThreeHoleDataPoint(point) ? formatValue(point.coefficients.K, 4) : '--' }}</td>
                    <td class="px-3 py-2 font-mono text-[var(--text-primary)]">{{ isThreeHoleDataPoint(point) ? formatValue(point.coefficients.Cv, 4) : '--' }}</td>
                    <td class="px-3 py-2 font-mono text-[var(--text-primary)]">{{ isThreeHoleDataPoint(point) ? formatValue(point.coefficients.Cp, 4) : '--' }}</td>

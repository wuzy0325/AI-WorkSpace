 ...
 * ）：
 *   - 所有阈值必须为正有限数（progressEpsilon 也要求 > 0，与后端 validateMotionSafetyFields 对齐）
 *   - noProgressTimeoutMs 必须 >= 200（后端 2*motionCompletePollMsForValidation=200）
 *   - criticalDeviationLimit 必须严格大于 arrivalTolerance（否则急停永远不可能触发，
 *     且偏离区间为空集，配置语义错误）
...
function getGlobalField<K extends keyof MotionSafetyConfig>(key: K): number | null {
  return (motionSafety.value?.[key] as number | undefined) ?? null
}
function setGlobalField<K extends keyof MotionSafetyConfig>(key: K, v: number | null): void {
...
function getAxisField(axis: string, key: keyof MotionSafetyConfig): number | null {
  return (motionSafety.value?.axisOverrides?.[axis]?.[key] as number | undefined) ?? null
}
function setAxisField(axis: string, key: keyof MotionSafetyConfig, v: number | null): void {
...
// ---- 实时校验 ----
// 校验返回错误消息字符串数组；空数组表示通过。
// 校验规则与后端 validateMotionSafetyConfig + Resolve 对齐：
//   - 单字段范围校验（正数 / >= 200 / < 120000）
//   - 同对象内跨字段校验（critical > arrival, epsilon < arrival）
//   - 按轴覆盖字段同样校验上述规则
//   - 每个绑定轴 Resolve(axis) 后的合并值校验 critical > arrival（防止组合倒置）
...
/** 按轴覆盖字段读取辅助 */
function getAxisFieldTyped(axis: string, key: keyof MotionSafetyConfig): number | null {
...
const validationErrors = computed<string[]>(() => {
  ...
  // 1. 全局字段范围校验（与后端 validateMotionSafetyFields 对齐）
  if (a !== null && a <= 0) errors.push(props.t.travMotionSafetyErrArrivalPositive)
  if (c !== null && c <= 0) errors.push(props.t.travMotionSafetyErrCriticalPositive)
  if (n !== null && n <= 0) errors.push(props.t.travMotionSafetyErrTimeoutPositive)
  if (n !== null && n < MIN_NO_PROGRESS_TIMEOUT_MS) errors.push(props.t.travMotionSafetyErrTimeoutMin200)
  if (e !== null && e <= 0) errors.push(props.t.travMotionSafetyErrEpsilonPositive)

  // 2. 全局 criticalDeviationLimit 必须严格大于 arrivalTolerance
  //    （两者都配置时，否则"超差区间"为空集，配置语义错误）
  if (a !== null && c !== null && c <= a) {
    errors.push(props.t.travMotionSafetyErrCriticalGreaterThanArrival)
  }

  // 3. 全局 progressEpsilon 应小于 arrivalTolerance
  //    （否则微动卡死会被误判为"有进展"，仅警告不阻断）
  if (a !== null && e !== null && e >= a) {
    errors.push(props.t.travMotionSafetyErrEpsilonLessThanArrival)
  }

  // 4. 全局 noProgressTimeoutMs 应显著小于 waitForMotionComplete 的 120s 兜底超时
  //    （否则看门狗永远不会先于兜底超时触发，形同虚设）
  if (n !== null && n >= 120000) {
    errors.push(props.t.travMotionSafetyErrTimeoutLessThan120s)
  }

  // 5. 按轴覆盖字段范围校验（与全局规则一致）
  for (const ax of props.motionAxes) {
    ...
    if (oa !== null && oa <= 0) errors.push(`[${ax.axis}] ${props.t.travMotionSafetyErrArrivalPositive}`)
    if (oc !== null && oc <= 0) errors.push(`[${ax.axis}] ${props.t.travMotionSafetyErrCriticalPositive}`)
    if (on !== null && on <= 0) errors.push(`[${ax.axis}] ${props.t.travMotionSafetyErrTimeoutPositive}`)
    if (on !== null && on < MIN_NO_PROGRESS_TIMEOUT_MS) errors.push(`[${ax.axis}] ${props.t.travMotionSafetyErrTimeoutMin200}`)
    if (oe !== null && oe <= 0) errors.push(`[${ax.axis}] ${props.t.travMotionSafetyErrEpsilonPositive}`)
  }

  // 6. 每个绑定轴 Resolve(axis) 后的合并值校验 critical > arrival

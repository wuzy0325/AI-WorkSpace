    ...
    <template #header>
      <div class="overview-panel__header-row flex min-w-full items-start justify-between gap-4">
        ...
        <div class="flex items-center gap-2">
          <UiButton variant="secondary" size="sm" class="overview-panel__action-btn" @click="() => { (deviceStore.profiles ?? []).filter(p => p.type === 'DAQ-P-1604' || p.type === 'DAQ-P-1604Pre' || p.type === 'DAQ-P-1603').forEach((p) => deviceStore.tareAllEnabled(p.id)) }">

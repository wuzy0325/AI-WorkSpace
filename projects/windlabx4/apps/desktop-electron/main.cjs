// WindLabX4 (Win7) Electron 主进程入口
//
// 职责：
//   1. spawn Go 后端 windlabx4-backend.exe（监听 127.0.0.1:8900）
//   2. 等待后端健康检查通过后，加载主窗口（http://127.0.0.1:8900）
//   3. 通过 IPC 处理文件对话框（pickDirectory）和运动控制器独立窗口（openMotionWindow）
//
// 安全约束（与 wista Win7 基线一致）：
...
//
// 注意：motion-only 子进程也是同一份 windlabx4-backend.exe，通过 --motion-only flag 启动，
...
// 后端二进制路径：
// - 开发态：desktop-electron/backend/windlabx4-backend.exe（由 build-backend.ps1 生成）
// - 打包后：resources/backend/windlabx4-backend.exe
function backendPath() {
  if (app.isPackaged) {
    return path.join(process.resourcesPath, 'backend', 'windlabx4-backend.exe')
  }
  return path.join(__dirname, 'backend', 'windlabx4-backend.exe')
}
...
function startMainBackend() {
  ...
  mainBackendProcess.once('error', (error) => {
    dialog.showErrorBox('WindLabX4 后端启动失败', error.message)
    app.quit()
  })
  mainBackendProcess.once('exit', (code) => {
    mainBackendProcess = null
    if (!app.isQuitting && code !== 0) {
      dialog.showErrorBox('WindLabX4 后端异常退出', `退出代码: ${code}`)
      app.quit()
    }
  })
}
...
// 流程：
//   1. spawn windlabx4-backend.exe --motion-only --parent-pid=<主进程 PID>
//   2. 等待 8901 健康检查通过
...
app.whenReady().then(async () => {
  ...
  try {
    ...
    dialog.showErrorBox('WindLabX4 启动失败', error.message)
    app.quit()
  }
})

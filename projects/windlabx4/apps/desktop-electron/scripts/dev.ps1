...
try {
    ...
    $backendProc = Start-Process -FilePath (Join-Path $goRoot 'bin\go.exe') `
        ...
        -RedirectStandardOutput "$env:TEMP\windlabx4-backend.out.log" `
        -RedirectStandardError "$env:TEMP\windlabx4-backend.err.log"
    ...
    if (-not $backendReady) {
        throw 'Go 后端 20 秒内未就绪，请查看日志: $env:TEMP\windlabx4-backend.err.log'
    }
    ...
    Write-Host '  - 后端日志: $env:TEMP\windlabx4-backend.err.log' -ForegroundColor Gray
    Write-Host ''

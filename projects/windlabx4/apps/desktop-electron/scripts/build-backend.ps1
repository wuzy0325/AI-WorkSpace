...
# 构建 WindLabX4 Win7 后端二进制：
#   1. 先 npm run build 前端到 desktop-wails/frontend/dist
#   2. 用 Go 1.20.14 编译 desktop-wails 为单一二进制（内嵌前端资源）
#   3. 输出到 desktop-electron/backend/windlabx4-backend.exe
# 与 wista 的 build-backend.ps1 保持一致的步骤与参数。
...
try {
    & (Join-Path $goRoot 'bin\go.exe') build -buildvcs=false -trimpath -ldflags '-s -w -H=windowsgui' -o (Join-Path $backendOutput 'windlabx4-backend.exe') .

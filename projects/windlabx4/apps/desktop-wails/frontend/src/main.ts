...
function initializeServices(): void {
  ...
  console.debug('[WINDLABX4] Feedback service initialized')
  
  const motion = container.motion
  console.debug('[WINDLABX4] Motion service initialized')
  
  if (import.meta.env.DEV) {
    (window as any).__WINDLABX4_CONTAINER__ = container
    console.debug('[WINDLABX4] Container exposed to window for debugging')
  }
}
...
async function bootstrap(): Promise<void> {
  ...
  if (isWailsAvailable()) {
    try {
      ...
      console.warn('[WINDLABX4] 获取启动模式失败，使用默认路由', e)
    }
  }

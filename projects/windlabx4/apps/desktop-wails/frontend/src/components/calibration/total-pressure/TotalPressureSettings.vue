...
const savePath = ref('')
const saveFileName = ref(buildCalibrationCsvName(calibrationName.value.trim(), 'total-pressure'))
...
// 球罐压力通道：仅用于前端实时显示压力值，不参与闸门判定；默认 channelIndex=-1 强制操作员手动选择
const sphereTankPressureChannel = ref<ChannelRef>({ deviceId: '', channelIndex: -1 })
...
const currentStepErrors = computed<string[]>(() => {
  ...
  if (currentStep.value === 0) {
    ...
    if (!saveFileName.value.trim()) errs.push(t.value.tp_pleaseInputCsvFileName)
  }
...
watch(calibrationName, (next, prev) => {
  const prevDefault = buildCalibrationCsvName(prev.trim(), 'total-pressure')
  if (saveFileName.value === '' || saveFileName.value === prevDefault)
    saveFileName.value = buildCalibrationCsvName(next.trim(), 'total-pressure')
})
      ...
      <div v-if="currentStep === 0" class="step-content">
        ...
        <UiPanel class="section-card">
          ...
          <div class="save-path-row">
            <UiInput v-model="savePath" :placeholder="t.tp_clickToPickSaveDir" class="flex-1" :title="savePath" />
            <UiInput v-model="saveFileName" :placeholder="t.tp_csvFileNamePlaceholder" class="flex-1" />
        ...
        <UiPanel class="section-card">
          ...
          <div v-if="sphereTankGateEnabled" class="sphere-grid">
            ...
            <div class="field"><span class="field-label">{{ t.wf_spherePressureChannel }}</span><UiSelect
              ...
              @update:model-value="sphereTankPressureChannel.channelIndex = $event !== '' ? Number($event) : -1"
              :options="channelIndexOptions"
              :placeholder="t.tp_selectChannel"
            /></div>

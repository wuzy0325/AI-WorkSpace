...
const latestCoefficients = computed(() => {
  if (calibrationStore.realtimeCoefficients && typeof calibrationStore.realtimeCoefficients !== 'number' && 'Kb' in calibrationStore.realtimeCoefficients) return calibrationStore.realtimeCoefficients
...
watch(isLoading, (loading) => {
  ...
  unsubscribeDaqSnapshot = deviceApi.onSnapshot((payload: DataPayload) => {
    ...
    if (pressures) {
      ...
      const requiredRoles = ['threeHole.p1', 'threeHole.p2', 'threeHole.p3', 'threeHole.pAtm']
      ...
      if (realtimeReady) {
        void calibrationStore.updateRealtimeCoefficients('three-hole', {
          P1: pressures.P1, P2: pressures.P2, P3: pressures.P3,
        ...
        <div class="flex-shrink-0 border-t border-[var(--border-default)] p-3">
          <div class="flex items-center gap-2 whitespace-nowrap rounded-lg bg-[var(--bg-panel-strong)] px-3 py-2 text-xs">
            ...
            <div class="flex items-center gap-2 whitespace-nowrap">
              <span class="text-[var(--text-muted)]">{{ t.th_sphereTankGate }}</span>

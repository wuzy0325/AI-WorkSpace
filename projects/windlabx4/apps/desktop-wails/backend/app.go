...
// %APPDATA%/WindLabX4/goroutine-dump-<step>.txt，用于定位退出卡死点。
...
// dumpGoroutines 将全 goroutine 栈写入 %APPDATA%/windlabx4/goroutine-dump-*.txt。
// 仅用于诊断退出卡死；文件名含时间戳避免覆盖历史记录。
func dumpGoroutines(reason string) {
	buf := make([]byte, 4*1024*1024)
	n := runtime.Stack(buf, true)
	content := fmt.Sprintf("WindLabX4 goroutine dump\n原因: %s\n时间: %s\n\n%s\n",
		reason, time.Now().Format("2006-01-02 15:04:05.000"), buf[:n])
	configDir, err := os.UserConfigDir()
	if err != nil {
		return
	}
	dir := filepath.Join(configDir, "windlabx4")
...
func (a *App) GetInstallerLanguage() string {
	keyPath := fmt.Sprintf(`Software\%s\%s`, "WindLabX4", "WindLabX4")
...
// 首次启动时若检测到旧目录 %APPDATA%\wind-daq 存在且新目录不存在，
// 自动将旧配置迁移到新目录，保证老用户升级后配置不丢失。
func writableUserConfigDir() (string, error) {
	configDir, err := os.UserConfigDir()
	if err != nil {
		return "", err
	}
	newDir := filepath.Join(configDir, "windlabx4")
	oldDir := filepath.Join(configDir, "wind-daq")
	if _, err := os.Stat(oldDir); err == nil {
		if _, err := os.Stat(newDir); os.IsNotExist(err) {
			slog.Info("migrating config from wind-daq to windlabx4", "from", oldDir, "to", newDir)
			if err := migrateConfigDir(oldDir, newDir); err != nil {
				slog.Warn("config migration failed, fall back to new dir", "error", err)
			}
		}
	}
...
// migrateConfigDir 将旧配置目录内容递归复制到新目录（仅迁移一次）。
// 使用复制而非移动，避免迁移过程中源目录被部分占用导致数据丢失。
...
func (a *App) CalibrationStart(dto types.CalibrationConfigDTO) GenericResponse {
	...
	return a.callMgr(a.calibrationManager(), "校准管理器", func() error {
		// 路径归一：相对 savePath 解析到 %APPDATA%\WindLabX4\<相对>，

...
// Stop 双探针 registry 关停顺序与 fatal seam（spec FR9 / Task 14）：
...
func TestStop_RegistryFailureFatalExitSeam(t *testing.T) {
	var exitCodes []int
	relayStopped := false
	app := &App{
...
func TestStop_RegistrySuccessContinuesCleanup(t *testing.T) {
	exitCalled := false
...
func TestStop_NoRegistryProceeds(t *testing.T) {
...
func TestNewApp_ConfiguresExitWatchdog(t *testing.T) {
	// NewApp 必须配置宿主退出 watchdog seam，避免优雅退出超时后进程残留。
	app := NewApp(ModeNormal)
	if app.exitWatchdog == nil {
		t.Fatal("NewApp 应配置 exitWatchdog seam（由退出路径触发）")
	}
}

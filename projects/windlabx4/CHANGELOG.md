...
- `go test ./internal/adapters/interpolation/ -run SevenHole`（WindLabX4 services/api-go）: passed
- `go vet ./...` + `go test ./internal/... ./api/...`（WindLabX4 services/api-go）: passed
- `npm run typecheck` / `npm run build`: passed
- `task release`: passed
- `makensis`: passed
- `task archive-release`: passed
...
- 本版本无 WindLabX4 自有 bug 修复，仅因共享算法包新增 Velocity 输出重新打包。

### Compatibility

- 配置文件格式：兼容。
- 数据文件格式：兼容（CSV 暂未新增 Velocity 列，未来 WindLabX4 校准 CSV 接入时再扩展）。
...
- `task release`: passed
- `makensis '-DARG_WAILS_AMD64_BINARY=..\..\bin\WindLabX4.exe' project.nsi`（在 `build/windows/installer/` 目录下执行）: passed
- `task archive-release`: passed

### Known Issues

- 安装包未进行 Authenticode 数字签名，Windows 可能显示"未知发布者"提示。
- WindLabX4 前端校准 CSV 暂未接入 Velocity 列输出，仍由前端独立计算；后续接入时统一来源。
...
- 本版本无 WindLabX4 自有代码变更，仅因共享 device-sdk 修复重新打包。
...
- 旧版本（≤0.12.1）遗留的 traversal checkpoint 文件在升级后会保留在磁盘但不会被读取，可手动清理 `%APPDATA%\WindLabX4\` 下的相关 json 文件。
...
- 设备网络操作防本机卡死整改（LSP 环境加固，对应 `b8475f4`）：LSP/安全软件（Astrill ASProxy64.dll、深信服驱动等）hook winsock 导致 SetReadDeadline 失效、conn.Close() 在挂起 I/O 时永久阻塞的故障模式下，退出/扫描/设备操作不再卡死主线程：
  ...
  - `app.go`：callMgr 与 DeviceScanDevices 增加 10s 有界等待（超时返回"操作超时"提示，前端可重试）；startLocalAPIServer 探测 127.0.0.1:8900 端口占用并弹窗提示；API server Shutdown 改后台 goroutine + 主线程 300ms 有界等待；退出确认后 30s exitWatchdog 强制结束进程；runShutdownStep 超 5s 自动 dump goroutine 栈到 `%APPDATA%\WindLabX4\goroutine-dump-*.txt`。
...
- 新增 `docs/plans/2026-07-31-daq-p1604-stop-transaction-remediation.zh-CN.md`：DAQ-P-1604 Stop 事务整改方案（待实现），涉及独立应用 / WindLabX4 / 共享协议三层。
- 新增 `docs/udp-discovery-windows-timeout-root-cause.md`：Windows UDP 设备扫描不返回问题根因与修复方案。
- 新增 `projects/WindLabX4/docs/test-cases.html`（181KB）：WindLabX4 功能测试用例文档，按用户测试用例格式要求编写（测试前置 / 测试步骤 / 期待结果三段式），用例优先级色板（P0/P1/P2）与结果色（pass/fail/block/skip）解耦，技术术语站在测试人员和用户角度改写。
...
- 修复 `projects/WindLabX4/services/api-go/internal/usecase/traversal_registry_recovery.go` 的 `LoadCheckpoint` 在 registry 仍持有活动 session 时返回磁盘 checkpoint 导致前端同时看到"正在运行"的 session 和"继续/放弃"提示造成状态分裂。
...
- `makensis -DARG_WAILS_AMD64_BINARY=build/bin/WindLabX4.exe build/windows/installer/project.nsi`: passed
...
- 前端 `SevenHoleCharts.vue` 完成 ECharts i18n 完整覆盖：axis / tooltip / legend / 标题全部走 i18nStore，延续 daq-p1604 / daq-t1603 / WindLabX4 三项目 i18n 改造工作。
...
- `task release`: passed (production build with -tags production, GOWORK=off)
- `makensis -DARG_WAILS_AMD64_BINARY=build/bin/WindLabX4.exe build/windows/installer/project.nsi`: passed (6.95 MB installer)
- `task archive-release`: passed (archived to releases/bin/)
...
- 完成 WindLabX4 全量代码审查整改，覆盖校准生命周期、并发访问、数据校验与错误传播等问题。
...
- 修复点位于 shared/device-sdk 共享代码，WindLabX4 的 T1603 设备适配器自动受益。
...
### Fixed
- 修复 WindLabX4 T1603Adapter.Connect() 自死锁：适配器持 a.mu 调 dev.Connect()，而同步 OnConfigSynced 回调重入 a.mu 导致永久死锁。

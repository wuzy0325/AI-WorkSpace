...
Charts in `WindLabX4` use **ECharts 6** (`echarts`, `vue-echarts`) plus custom canvas where ECharts is too heavy. This is the chosen library; do not introduce a second charting toolkit without a written decision in `docs/decisions/`.
...
A WindLabX4 chart has at most five layered regions, drawn back-to-front:

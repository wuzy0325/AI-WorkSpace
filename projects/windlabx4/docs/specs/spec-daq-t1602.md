# Spec: DAQ-T1602 温度扫描阀设备集成到 WindLabX4
...
将 DAQ-T1602 温度扫描阀集成到 WindLabX4，作为新的 DAQ 设备类型。T1602 是 **Modbus TCP** 温度采集设备：机内含 2 张独立采集卡（不同网卡、不同 Slave ID），每卡 8 通道，合并对外为 16 通道单设备。

### 与现有 DAQ-T-1603 的身份隔离（Critical）

WindLabX4 已存在 **DAQ-T-1603**（`Type = "DAQ-T-1603"`，裸 TCP ASCII 协议 + BIN 二进制）。T1602 与 T1603 协议、传输方式、数据语义完全不同，必须严格隔离：
...
1. DAQ-T-1602 可作为设备类型添加到 WindLabX4，前后端 `Type = "DAQ-T-1602"` 一致，与 T1603 完全隔离
...
- **全通道类型设置（2026-08-19）**：`DaqT1602Config.vue` 面板顶部提供"全通道类型"下拉，选择后一次性应用到全部 16 通道并复位（一次性操作，避免与后续单通道修改产生陈旧显示）。
- **热电偶范围显示（2026-08-19）**：面板底部图例展示固件量程表全部 8 型范围（℃）；通道下拉 hover 显示该类型量程（`UiSelect` 选项可选 `title` 字段，向后兼容）。
- **T1602 独立采样频率（2026-08-19 定案）**：T1602 配置面板新增"采样频率"（1~5 Hz，`DaqT1602HardwareConfig.SampleRateHz`，默认 5），控制采集/保存频率——驱动每次采样前动态读取该配置，轮询间隔 = 1000/hz ms，设置变更即时生效、无需重启采集。设 1Hz 就每秒读存 1 帧完整 16 通道快照，设 5Hz 全速 ~4.9Hz（受设备单帧 ~206ms 限制自动追赶，不积压）。**与全局"刷新频率"（1~10 Hz，界面显示节奏）完全独立**：全局刷新频率是所有设备共用的显示设置，不得因 T1602 修改；T1602 采集节奏只由其专属采样频率决定。链路：`前端 DaqT1602Config.sampleRateHz → profile.daqT1602Config → 适配器 SetPollIntervalFn（帧节拍器，daq_t1602_poll.go）`。其他设备（T1603/P1604 等）不受影响、保持各自采样率全量保存。
...
2. `go build -buildvcs=false ./...` + `gofmt -l .` 无输出 + `go vet ./...` + `go test ./internal/... ./api/...` 全部通过（WindLabX4）
...
- **Q5（通道掩码/采样率）**：T1602 文档无采样率/通道掩码/触发概念（固件固定 ~100ms 周期），确认不需要 T1603 那些配置项。**2026-08-19 补充**：虽然硬件采集周期固定，但"读取/保存"频率由软件轮询间隔控制，已新增 T1602 专属"采样频率"设置（1~5 Hz，独立于全局刷新频率），设置即采集频率，见 §前端集成约定。

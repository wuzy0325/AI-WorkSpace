# Implementation Plan: MPS-03 集成到 WindLabX4
...
将 MPS-03 多功能探针设备作为新 DAQ 设备类型集成到 WindLabX4。采用 shared SDK + thin wrapper 双层架构：协议层和驱动层放 `shared/device-sdk/go/`，WindLabX4 内做类型翻译。集成范围涵盖 TCP 通信、9 项硬件配置同步、16 通道 CSV 数据流解析、断连重连状态机、专属 CSV 17 列表头、配置 UI 面板、HTTP/Wails API。
...
| AD2 | shared SDK + thin wrapper 双层 | 跨项目复用（参照 DAQ-P-1603 模板 C）；shared SDK 禁止依赖 WindLabX4 internal | 仅 WindLabX4 adapter（无法复用） |
...
| AD5 | 重连状态机内嵌 shared SDK 驱动层 | 状态机与设备生命周期紧耦合；DeviceManager 无重连能力 | WindLabX4 adapter 层重连（需跨层协调） |
...
- [ ] 协议层无 WindLabX4 依赖（`grep -r "WindLabX4" shared/device-sdk/go/protocol/` 无命中）
...
- [ ] 手动启动模拟器 + WindLabX4 前端可完成全流程（Phase 5 验证）
...
- [ ] `grep -r "WindLabX4" shared/device-sdk/go/` 无命中（依赖方向验证）
...
#### Task 7: WindLabX4 core/ports 镜像

**Description**：在 WindLabX4 的 `core/device/types.go` 和 `ports/device.go` 镜像 shared SDK 的 MPS-03 类型与接口定义，作为 adapter 翻译的本地类型。
...
- [ ] `ports.MPS03Configurable` 接口定义（与 shared SDK 镜像，但用 WindLabX4 类型）
- [ ] 编译通过：`cd projects/WindLabX4/services/api-go && go build ./...`

**Verification**:
- [ ] `cd projects/WindLabX4/services/api-go && go build ./... && go vet ./...`
- [ ] `grep -r "DeviceMPS03\|MPS03Configurable\|MPS03HardwareConfig" projects/WindLabX4/services/api-go/internal/core/ projects/WindLabX4/services/api-go/internal/ports/` 命中位置正确

**Dependencies**: T3（shared SDK 类型定义先行）

**Files likely touched**:
- `projects/WindLabX4/services/api-go/internal/core/device/types.go`（修改）
- `projects/WindLabX4/services/api-go/internal/ports/device.go`（修改）
...
**Description**：实现 WindLabX4 适配器 `MPS03Adapter`，委托给 shared SDK 的 `MPS03Device`，负责 WindLabX4 Profile ↔ shared SDK Profile、Status、DataPayload 的双向类型翻译。
...
- [ ] `SetDataSink(sink)` 包装 WindLabX4 sink 为 shared sink 并注册到 driver
- [ ] `SetOnError(cb)` 委托 driver.SetOnError
- [ ] `Status()` 翻译 shared status → WindLabX4 status
- [ ] `GetHardwareConfig` / `ApplyHardwareConfig` 委托 + 类型翻译
- [ ] DataPayload 翻译：shared.DataPayload → WindLabX4.DataPayload（含 Channels/ChannelIndices）

**Verification**:
- [ ] `cd projects/WindLabX4/services/api-go && go test ./internal/adapters/hardware/... -run MPS03 -v`
- [ ] `go vet ./...` 通过
...
**Files likely touched**:
- `projects/WindLabX4/services/api-go/internal/adapters/hardware/mps03_adapter.go`（新增）
- `projects/WindLabX4/services/api-go/internal/adapters/hardware/mps03_adapter_test.go`（新增，可选）
...
**Files likely touched**:
- `projects/WindLabX4/services/api-go/internal/adapters/config/default_profiles.go`（修改）
- `projects/WindLabX4/services/api-go/internal/bootstrap/bootstrap.go`（修改）
...
**Verification**:
- [ ] `cd projects/WindLabX4/services/api-go && go test ./api/... -run MPS03 -v`
- [ ] 测试用例：GET 成功、PUT 成功、PUT 采集中 409、PUT 校验失败 400、GET 设备未连接 503

**Dependencies**: T8, T9

**Files likely touched**:
- `projects/WindLabX4/services/api-go/api/server.go`（修改）
...
**Verification**:
- [ ] `cd projects/WindLabX4/services/api-go && go test ./internal/adapters/storage/... -run MPS03 -v`
...
**Files likely touched**:
- `projects/WindLabX4/services/api-go/internal/adapters/storage/csv_sink.go`（修改）
...
**Files likely touched**:
- `projects/WindLabX4/services/api-go/api/server_test.go`（修改或新增 MPS03 用例段）

**Estimated scope**: M（1 文件，但用例多）

---

### Checkpoint: Phase 3 WindLabX4 项目层完成
...
**Verification**:
- [ ] `cd projects/WindLabX4/apps/desktop-wails/frontend && npm run typecheck`
- [ ] `npm run build` 通过

**Dependencies**: T10（API 端点先行，提供契约）

**Files likely touched**:
- `projects/WindLabX4/apps/desktop-wails/frontend/src/api/types.ts`（修改）
- `projects/WindLabX4/apps/desktop-wails/frontend/src/api/deviceApi.ts`（修改）
...
**Files likely touched**:
- `projects/WindLabX4/apps/desktop-wails/frontend/src/components/device/Mps03ConfigPanel.vue`（新增）
...
**Files likely touched**:
- `projects/WindLabX4/apps/desktop-wails/frontend/src/components/device/DeviceManagementDrawer.vue`（修改）
...
**Files likely touched**:
- `projects/WindLabX4/apps/desktop-wails/frontend/src/stores/deviceStore.ts`（修改）
...
**Verification**:
- [ ] `cd projects/WindLabX4/apps/desktop-wails && wails3 generate bindings -silent`
- [ ] `cd projects/WindLabX4/services/api-go && go build ./... && go vet ./...`
- [ ] `cd projects/WindLabX4/apps/desktop-wails/frontend && npm run typecheck && npm run build`
...
**Files likely touched**:
- `projects/WindLabX4/apps/desktop-wails/backend/app.go`（修改）
- 自动生成的 TS binding 文件
...
**Description**：使用 MPS-03 模拟器 + WindLabX4 完成端到端验证，覆盖 spec 全部 16 项成功标准，并在真实硬件上验证 CHA != FFFF 契约。
...
- [ ] shared SDK + WindLabX4 双模块 `go test ./... && go vet ./...` 全绿
...
| R5 | shared SDK 误导入 WindLabX4 internal | High | 低 | T1-T6 checkpoint 用 grep 验证；`validate-structure.ps1` 兜底 |
...
|---|---|---|---|
| C1 | Phase 1 后 | 协议层编译 + 测试 + 依赖方向 | `go test ./protocol/...` 全绿 + grep 无 WindLabX4 依赖 |
| C2 | Phase 2 后 | shared SDK 全部测试 + 命名隔离 | `go test ./...` 全绿 + grep 无 DAQP1603 |
| C3 | Phase 3 后 | WindLabX4 全部测试 + 命名隔离 | `go test ./...` 全绿 + grep 无 DAQP1603 |

...
Phase 2  后端集成（projects/windlabx4/services/api-go）—— 依赖 Task 6 通过
...
| CP3 后端集成 | Phase 2 每个任务 | `cd projects\WindLabX4\services\api-go; go test ./internal/...`（含既有五孔用例）；`importSevenHolePrb` / `calculateRealtime`（P7 体）curl 冒烟 | 新增用例通过；既有用例零失败；curl 返回结构符合 spec §5.6 |

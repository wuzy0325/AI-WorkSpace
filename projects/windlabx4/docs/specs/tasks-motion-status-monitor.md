...
**Verification:**
- [ ] 测试通过：`cd projects\WindLabX4\services\api-go; go test ./internal/adapters/motion/... ./internal/ports/...`
...
**Files likely touched:**
- `projects/windlabx4/services/api-go/internal/ports/motion.go`
- `projects/windlabx4/services/api-go/internal/adapters/motion/wrapper.go`
- `projects/windlabx4/services/api-go/internal/adapters/motion/wrapper_test.go`
...
- `projects/windlabx4/apps/desktop-wails/backend/motion_bindings.go`（Wails MotionGetStatus）
...
**Description:** 改造 `projects/windlabx4/services/api-go/internal/usecase/calibration.go` 的 `fallbackRuntime.WaitForMotionComplete`（L1082-1165），从 100ms `motionCompletePoll` ticker + `StatusAll` 迁移为 `MotionStatusReader.WaitNext` + `JudgeArrival`；处理 `ErrGenerationChanged` 时 abort 当前点；保持既有到位/NoProgress/StatusUnavailable 语义不变。
...
**Verification:**
- [ ] 测试通过：`cd projects\WindLabX4\services\api-go; go test ./internal/usecase/... -run 'TestCalibration.*Motion'`
...
**Description:** 改造 `projects/windlabx4/services/api-go/internal/usecase/traversal_acquisition.go` 的 `waitForMotionComplete`（L1056-1155）与 `recheckMotionSafety`（L854-904），从 100ms ticker + `StatusAll` 迁移为 `WaitNext` + `JudgeArrival`；处理 `ErrGenerationChanged` 时 abort 当前点；保持 fixed/adaptive stabilization 语义不变。
...
- `projects/windlabx4/services/api-go/internal/usecase/traversal_acquisition_test.go`
...
**Description:** 改造 `projects/windlabx4/apps/desktop-wails/frontend/src/api/motionApi.ts` 与 `stores/motionStore.ts`，确保每个进程内最多保持一条状态轮询/订阅链；`attachStatusListener()` 只注册观察者，不创建新的后端轮询链；`refreshStatus()` 仅用于首次加载/显式刷新。
...
**Verification:**
- [ ] 测试通过：`cd projects\WindLabX4\apps\desktop-wails\frontend; npm run test -- --grep 'motionApi|motionStore'`
...
**Files likely touched:**
- `projects/windlabx4/apps/desktop-wails/frontend/src/api/motionApi.ts`
- `projects/windlabx4/apps/desktop-wails/frontend/src/stores/motionStore.ts`
- `projects/windlabx4/apps/desktop-wails/frontend/src/api/motionApi.test.ts`（新增）
- `projects/windlabx4/apps/desktop-wails/frontend/src/stores/motionStore.test.ts`（新增）
...
**Files likely touched:**
- `projects/windlabx4/apps/desktop-wails/frontend/src/components/calibration/five-hole/FiveHoleMain.vue`
- `projects/windlabx4/apps/desktop-wails/frontend/src/components/calibration/three-hole/ThreeHoleMain.vue`
- `projects/windlabx4/apps/desktop-wails/frontend/src/components/calibration/total-pressure/TotalPressureMain.vue`
...
**Files likely touched:**
- `projects/windlabx4/apps/desktop-wails/frontend/src/api/motionApi.ts`
- `projects/windlabx4/apps/desktop-wails/frontend/src/components/motion/StandaloneDisconnectBanner.vue`（新增）
- `projects/windlabx4/apps/desktop-wails/frontend/src/composables/useStandaloneDisconnect.ts`（新增）
- `projects/windlabx4/apps/desktop-wails/frontend/src/api/motionApi.test.ts`
...
**Files likely touched:**
- `projects/windlabx4/apps/desktop-wails/frontend/src/components/motion/EmergencyStopUntrustedBanner.vue`（新增）
- `projects/windlabx4/apps/desktop-wails/frontend/src/composables/useEmergencyStopUntrusted.ts`（新增）
- `projects/windlabx4/apps/desktop-wails/frontend/src/stores/motionStore.ts`（订阅事件）
- `projects/windlabx4/apps/desktop-wails/frontend/src/api/motionApi.ts`（HTTP 轮询 /api/motion/emergency-status）
...
- [ ] 若需要 UI，新增最小迁移任务（沿用 WindLabX4 Task 17 的组件）
...
**Description:** `events.StartStatusPoller` 已在 WindLabX4 关闭（[app.go L141-144](file:///c:/Users/wuzhy/Documents/D/SVN/SoftWare/trunk/AI-Workspace/projects/windlabx4/apps/desktop-wails/backend/app.go)）；monitor 完成后该函数无消费者；删除或改造成 monitor 的薄启动包装，禁止并存两个后台采集循环。
...
- [ ] `cd projects\WindLabX4\services\api-go; go test ./internal/... ./api/...; go vet ./internal/... ./api/...; go build -buildvcs=false ./...`
- [ ] `cd projects\WindLabX4\apps\desktop-wails\frontend; npm run typecheck; npm run test; npm run build`
- [ ] `cd projects\WindLabX4\apps\desktop-wails; task check-bindings`
- [ ] `cd projects\motion-controller\services\api-go; go test ./...; go vet ./...; go build -buildvcs=false ./...`
- [ ] `cd projects\motion-controller\apps\desktop-wails; go test ./...; go vet ./...; go build -buildvcs=false ./...`
- [ ] `cd projects\motion-controller\apps\desktop-wails\frontend; npm run typecheck; npm run build`

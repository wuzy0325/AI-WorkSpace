# Spec: WindLabX4 实时波形图性能优化

## Objective（目标）

### 我们在解决什么问题
WindLabX4 设备详情面板的实时波形图（`RealtimeChart.vue`）在采集数据累积占满缓冲区后出现明显 UI 卡顿。根因是数据层 `O(n)` 数组搬移 + 渲染层每帧全量重算/全量 setOption + ECharts 平滑曲线与面积渐变的渲染开销叠加。
...
- 目标：仅 `projects/WindLabX4`，不动 `daq-t1603` / `daq-p1604`
...
> 不在 `shared/` 抽共享件——本次仅 WindLabX4 范围。若后续 daq-t1603/daq-p1604 要复用，再单独发起迁移 spec。

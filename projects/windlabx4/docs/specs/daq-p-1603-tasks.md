...
> Plan 参考：[daq-p-1603-plan.md](file:///c:/Users/wuzhy/Documents/D/SVN/SoftWare/trunk/AI-Workspace/projects/WindLabX4/docs/specs/daq-p-1603-plan.md)
> 决策表 D-1~D-7 已确认（见 spec §Open Questions 与 plan §7）

## Overview

在 WindLabX4 中新增 DAQ-P-1603 16 通道通用 AI 采集设备。设备特点：每通道可配置为压力或温度传感器；采样率 ≤500Hz；手动输入 IP 连接（不扫描）；CSV 表头按通道类型动态生成。采用 CGo + DLL FFI 调用 WTNDAQ16H_64.dll。

## Architecture Decisions

- **代码分层**：FFI 在 shared/device-sdk/go/ffi，完整驱动在 shared/device-sdk/go/daq/hardware，thin wrapper 在 WindLabX4，与现有 DAQ-T-1603 模式对齐
...
- [ ] **Task 2**: core 类型常量 + ChannelConfig.SensorType 字段
  - Description: 在 shared SDK 与 WindLabX4 两处 core 类型中新增 DAQ-P-1603 设备常量，并为 ChannelConfig 添加 SensorType 字段（默认 pressure 保证向后兼容）。
  - Acceptance:
    ...
    - [ ] `projects/WindLabX4/services/api-go/internal/core/device/types.go` 同步新增 `DeviceDAQP1603`
    - [ ] 不破坏现有 DAQ-P-1604 / DAQ-T-1603 测试
  - Verification:
    - [ ] `cd shared/device-sdk/go && go build ./... && go test ./...`
    - [ ] `cd projects/WindLabX4/services/api-go && go build ./... && go test ./...`
...
- [ ] **Task 4**: WindLabX4 thin wrapper（Connect/Disconnect 部分）
  - Description: 在 WindLabX4 内创建 thin wrapper，桥接 shared SDK DAQP1603 与 WindLabX4 ports.Device 接口。本任务仅实现连接相关方法。
  - Acceptance:
    - [ ] `projects/WindLabX4/services/api-go/internal/adapters/hardware/daq_p1603.go` 实现 WindLabX4 `ports.Device` 的连接方法
  ...
  - Dependencies: Task 3
  - Files likely touched:
    - `projects/WindLabX4/services/api-go/internal/adapters/hardware/daq_p1603.go`（新增）
...
- [ ] **Task 5**: bootstrap 注册 + profile + 手动 IP 输入 UI
  ...
  - Verification:
    - [ ] `cd projects/WindLabX4/services/api-go && go build ./... && go vet ./... && go test ./...`
  ...
  - Files likely touched:
    ...
    - `projects/WindLabX4/apps/desktop-wails/config/device-profiles.json`（修改）
...
- [ ] **Task 10**: WindLabX4 thin wrapper 扩展采集方法
  ...
  - Acceptance:
    ...
    - [ ] 数据 sink 桥接到 WindLabX4 的数据流
  - Verification:
    - [ ] `cd projects/WindLabX4/services/api-go && go build ./... && go vet ./...`
    - [ ] 编译期断言通过
  - Dependencies: Task 9
  - Files likely touched:
    - `projects/WindLabX4/services/api-go/internal/adapters/hardware/daq_p1603.go`（修改）
  - Estimated scope: Small（1 文件）
...
- [ ] **Task 12**: CSV recorder 扩展支持 DAQ-P-1603 动态表头
  - Description: 扩展 WindLabX4 的 CSV recorder，为 DAQ-P-1603 生成 17 列动态表头（Timestamp + 16 通道），每通道列名按 SensorType 生成（如 CH01_Pa、CH02_℃）。
  - Acceptance:
  ...
  - Dependencies: Task 9
  - Files likely touched:
    - `projects/WindLabX4/services/api-go/internal/adapters/storage/csv_sink.go`（修改，路径待确认）
    - 对应测试文件（修改）
  - Estimated scope: Medium（2 文件）
...
- [ ] **Task 16**: HIL 实机回归测试 + Wails 绑定同步
  ...
  - Verification:
    - [ ] 按 `projects/WindLabX4/docs/runbooks/hil-validation-plan.md` 执行

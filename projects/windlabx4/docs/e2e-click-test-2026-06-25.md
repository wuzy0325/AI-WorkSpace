...
- 测试范围：按当前配置打开 WindLabX4，依次巡检仪表盘、探针校准、遍历测试、运行日志、设置入口等页面的可见按钮。
- 原始记录：仓库根目录 `WindLabX4-e2e-click-audit.json`。
- 运动控制专项原始记录：仓库根目录 `WindLabX4-motion-click-audit.json`、`WindLabX4-motion-control-click-audit.json`、`WindLabX4-motion-control-exact-click-audit.json`。
- 运动控制真实 local API 用户流记录：仓库根目录 `WindLabX4-motion-user-flow-audit.json`。
- 剩余控件专项记录：仓库根目录 `WindLabX4-remaining-controls-audit.json`。
...
- 启动 `build/bin/WindLabX4.exe` 后，`127.0.0.1:8900/api/motion/status` 在崩溃前返回 200。
...
- 相关线索：`projects/WindLabX4/apps/desktop-wails/frontend/src/api/traversalApi.ts` 默认 API base 为 `http://127.0.0.1:8900`，与 README 中 Vite proxy 到 `:8080` 的当前开发配置不一致。
...
- 复现方式：运行 `build/bin/WindLabX4.exe`，应用初始化成功并打印 `WindLabX4 local API listening on http://127.0.0.1:8900`。
...
- 长期：建议在 `projects/WindLabX4/apps/desktop-wails/frontend` 引入官方 `@playwright/test`，增加 `playwright.config.ts`、trace/video/screenshot 留档，并把 `VITE_API_BASE` 与启动命令固化到 webServer 配置。

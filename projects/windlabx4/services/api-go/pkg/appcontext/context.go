...
import (
	...
	"windlabx4/services/api-go/internal/adapters/calstore"
	...
	"windlabx4/services/api-go/internal/adapters/report"
...
func NewAppContext(configDir string) (*AppContext, error) {
	if configDir == "" {
		...
		if err != nil {
			...
			// 首次启动自动迁移：老用户旧配置目录 %APPDATA%\wind-daq → windlabx4
			oldDir := filepath.Join(configDir, "wind-daq")
			configDir = filepath.Join(configDir, "windlabx4")
			migrateLegacyConfigDir(oldDir, configDir)
		}
	}
	...
	csvWriterInstance := storage.NewCalibrationCsvWriterOverwrite(calibration.Config{})
...
func (deviceFactory) Create(profile device.Profile) (windaqports.Device, error) {
	...
	case device.DevicePACE1000:
		return windaqhardware.NewPACE1000Adapter(profile), nil

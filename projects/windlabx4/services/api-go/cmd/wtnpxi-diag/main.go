...
//
// 协议定义（依据现场抓包 temp/log.txt 校准，可能偏离 WindLabX4 wtn_pxi.go 旧假设）：
//   - TCP 连接，设备主动推送数据流
...
// parseFrame 从 buffer 解析一帧 WTN-PXI 数据（对齐 WindLabX4 processData 的组帧/重同步逻辑）。
//
// 返回：
...
func printDetailFrame(frameNo uint64, payload []byte, st *toolState) {
	...
	if len(vals) < requiredValues {
		fmt.Printf("  [警告] 值数量 %d < %d（WindLabX4 要求至少 %d 路）\n",

...
import (
	...
	"windlabx4/services/api-go/internal/bootstrap"
...
func main() {
	...
	server, err := bootstrap.BuildAPIServer(bootstrap.Config{
		Address:          envOrDefault("WINDLABX4_ADDR", bootstrap.DefaultAddress),
		ProfileStorePath: envOrDefault("WINDLABX4_PROFILE_PATH", bootstrap.DefaultProfileStorePath),
	...
	slog.Info("windlabx4 api server starting", "addr", server.Address)
	select {
	case <-signalCtx.Done():
...
func initLogging() (*logging.Manager, error) {
	logDir := envOrDefault("WINDLABX4_LOG_DIR", filepath.Join("data", "logs"))

...
import { api, type PrbFileInfo, type LoadPrbResult, type InterpolationInput, type InterpolationResult } from './http-adapter'
...
function onPrbFilesSelected(event: Event) {
  const target = event.target as HTMLInputElement
  const files = target.files
  if (!files || files.length === 0) return
  doLoadPrb(Array.from(files))
  target.value = ''
}

async function doLoadPrb(files: File[]) {
  try {
    const [resp, result] = await api.loadPrbFiles(files)
    if (!resp.success) {
...
async function doImportCsv(file: File) {
  ...
  try {
    const [resp, data] = await api.importCsvData(file)
    if (!resp.success) {
...
<template>
  <div class="app">
    <header class="app-header">
      <div class="header-brand">
        ...
        <div class="brand-text">
          <h1>三孔探针插值计算 <span class="win7-badge">Win7</span></h1>
      ...
      <div class="header-actions">
        ...
        <label class="btn btn-primary" :class="{ disabled: calculating }">
          加载 PRB 文件
          <input type="file" multiple accept=".prb,.PRB" class="hidden-file-input" @change="onPrbFilesSelected" :disabled="calculating" />
        </label>
      </div>
    </header>
          ...
          <div class="input-section">
            <div class="section-header">
              ...
              <div class="section-actions">
                ...
                <label class="btn btn-secondary" :class="{ disabled: !loaded }">
                  导入数据
                  <input type="file" accept=".csv,.txt,.dat,.CSV,.TXT,.DAT" class="hidden-file-input" @change="onCsvFileSelected" :disabled="!loaded" />
                </label>
    ...
    <footer class="status-bar">
      ...
      <div class="status-meta"><span>三孔探针插值计算系统 v1.0 (Win7版)</span></div>
    </footer>
  </div>
</template>
...
.brand-text h1 { font-size: 18px; font-weight: 600; color: var(--gray-900); letter-spacing: -0.3px; display: inline; }
.subtitle { font-size: 12px; color: var(--gray-400); }

.win7-badge {
  font-size: 11px; font-weight: 500; background: var(--warning-50);
  color: var(--warning-600); padding: 1px 6px; border-radius: 4px;
...
.btn-primary:hover:not(:disabled):not(.disabled) { background: var(--primary-700); }
.btn-secondary { background: white; color: var(--gray-700); border: 1px solid var(--gray-200); }
.btn-secondary:hover:not(:disabled):not(.disabled) { background: var(--gray-50); border-color: var(--gray-300); }

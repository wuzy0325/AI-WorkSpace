...
- 完成Stop排空后，Start先精确消费单字节`A`，再从下一字节建立64字节数据边界。
- Stop后由唯一reader继续消费完整尾帧和事务边界上的单字节`A`，再清空应用层缓冲并排空socket至1秒静默加500ms确认完成，然后结束readLoop。
- ACK超时或边界无法确认时Close连接、进入Error并要求重连，不猜测或滑动数据边界。
- 禁止在普通采集路径中搜索 `0x41`、逐字节滑动或用温度合理性猜测 ACK/帧边界。
...
func startAcquisition():
    ...
    // 已建立干净事务边界：实机 ACK 为前导单字节 A。
    expectSingleByteACK("A")

...
- Source branch before promotion: `feature/wista-win7`
- Historical HEAD before Win7 commit: `b04103361e2c28360033886d7b6d4a60e24f1e27`
- Verified product: `wista 0.3.3`
...
| wista backend | `projects/wista/apps/desktop-wails/main.go`, `core/`, `backend/`, `httpserver/` | Replace Wails runtime with loopback HTTP and WebSocket events |
...
| Electron shell | `projects/wista/apps/desktop-electron/` excluding ignored artifacts | Start backend, load local UI, expose dialog IPC and build NSIS x64 |
...
The following paths are intentionally excluded by `projects/wista/apps/desktop-electron/.gitignore`:
...
No `WindLabX4` or `motion-controller` source changes were present in this worktree baseline. All 36 Git status entries were attributable to the groups above.
